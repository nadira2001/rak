...
#    return client_id
    ...
    # c_str = "SELECT cid,field2,created_by FROM sm_company_settings WHERE cid='"+cid+"' AND status='ACTIVE' LIMIT 1 "
    # c_Row=db.executesql(c_str,as_dict=True)
    # for c_Row in c_Row:
    #     field2 = str(c_Row['field2'])
    #     created_by = str(c_Row['created_by'])
    # if str(field2):
    #     return 'FAILED<SYNCDATA>'+str(created_by)
...
#     return cid
    ...
    # c_str = "SELECT cid,field2,created_by FROM sm_company_settings WHERE cid='"+cid+"' AND status='ACTIVE' AND field2='1'LIMIT 1 "
    # c_Row=db.executesql(c_str,as_dict=True)
    # for c_Row in c_Row:
    #     field2 = str(c_Row['field2'])
    #     created_by = str(c_Row['created_by'])
...
#                 repStr+='<li  style="border-bottom-style:solid; border-color:#CBE4E4;border-bottom-width:thin" ><input  type="submit" onClick="tourRepInfo(\''+rep_id+'\');"   style=" background-color:#09C; color:#FFF; font-size:12px; width:40px; height:30px;" value=" Info "    />&nbsp;&nbsp; <font id="'+ rep_id +'"  class="name" >'+ rep_id_name+'</font></li>'
                ...
                repStr+='<font style="color:#fff"  id="'+ rep_id +'" class="name" >'+ rep_id_name+' | '+rep_id+'   </font>   </td></tr>'
...
def diaryImage_submit():
    ...
    user_type = str(request.vars.user_type).strip().upper()
    # return user_type
    diary_image = str(request.vars.diaryPhoto).strip()
    diary_img_date = str(request.vars.diary_img_date).strip()
    ...
    if not compRow:
        ...
        if not repRow:
            ...
            insert_check_in = db.sm_diary_image.insert(cid=cid, rep_id=rep_id, user_type=user_type,rep_name=rep_name,diary_image=diary_image,diary_img_date=diary_img_date)

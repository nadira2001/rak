...
def check_user():
    ...
    if supervisorRows:
        ...
        c_name='KPL'; phone='123';  email_add='abc@kpl.com'; street='Kumudini'; city='Dhaka'; country='Bangladesh'; zip_code='1234'; logo=''
...
#                        menuDivStr="""<div id="accordion" style="width:180px;"> """
                        ...
                        if session.field_force == 1 or session.setting_working_area == 1 or session.utility_settings == 1:
                            if check_role('rm_workingarea_manage') == True or check_role('rm_workingarea_view') == True  or check_role('rm_rep_manage') == True  or check_role('rm_rep_view') == True  or check_role('rm_reparea_manage') == True  or check_role('rm_reparea_view') == True  or check_role('rm_sup_manage') == True  or check_role('rm_sup_view') == True   or check_role('rm_delivery_man_manage') == True or check_role('rm_delivery_man_view') == True or check_role('rm_utility_manage') == True or check_role('rm_ff_target_manage') == True or check_role('rm_ff_target_view') == True:
                                ...
                                menuDivStr += """<li ><a href="/""" + appName + """/visitPlan/index"><div>Visit Plan</div></a></li>"""
...
#                                if check_role('rm_doctor_visit_manage') == True or check_role('rm_doctor_visit_view') == True:
                                ...
                                if check_role('rm_doctor_visit_view') == True or check_role('rm_doctor_visit_view') == True:
                                        ...
                                        menuDivStr += """<li ><a href="/""" + appName + """/pr_report/index"><div>Prescription Report</div></a></li>"""
                                ...
                                #===============================29-01-19 Rima Start===========================
                                if check_role('farm_manage') == True or check_role('farm_view') == True or check_role('rm_doctor_visit_manage') == True or check_role('rm_doctor_visit_view') == True or check_role('rm_doctor_visit_plan_manage') == True or check_role('rm_doctor_visit_plan_view') == True or check_role('rm_doctor_inbox_manage') == True:
    
                                    menuDivStr+="""<div style=" background-color:#F2F2F2; border-radius:0px; padding:0px 0px 0px 0px; margin:0px; font-size:16px; height:25px; vertical-align:middle; background-image: linear-gradient( #F5F5F5 , #CACACA 50px);"><div id="doctor">&nbsp;Farm</div></div>"""
                                    menuDivStr+="""<div style="margin:0px; padding:0px; border:0px;">"""
                                    menuDivStr+="""<ul>"""
                                    menuDivStr+="""<li style="background-color:#D8EBEB;font-weight:bold"><a href="/"""+appName+"""/farm/farm_visit"><div>Farm</div></a></li>"""
                                    menuDivStr+="""</ul>"""
                                    
                                    menuDivStr+="""<ul>"""
                                    menuDivStr+="""<li style="background-color:#D8EBEB;font-weight:bold"><a href="/"""+appName+"""/farm/farm_visit_detail"><div>Farm Visit</div></a></li>"""
                                    menuDivStr+="""</ul>"""
                                    menuDivStr+="""</div>"""
    #===============================29-01-19 Rima End===========================
...
#                                     menuDivStr += """<li ><a href="/""" + appName + """/dashboard/dashboard"><div>Dashboard</div></a></li>"""
                                ...
                                menuDivStr += """<li ><a href="/""" + appName + """/tracking/tracking"><div>Tracking</div></a></li>"""
...
#    my_date=get_mydate(gmt_time=session.gmt)
    ...
    update_zone="""update sm_tracking_table t,sm_level l set  t.zone_id=l.level0 where t.cid=l.cid and t.area_id=l.level3 and t.visit_date = '"""+str(current_date)+"""'"""
    update_smsStatus_run=db.executesql(update_zone)     
    
    qset=db()
    qset=qset(db.sm_tracking_table.cid == c_id) 
    qset=qset(db.sm_tracking_table.visit_date == current_date) 

    recFirst_submit=qset.select(db.sm_tracking_table.zone_id,db.sm_tracking_table.rep_id,db.sm_tracking_table.rep_name,db.sm_tracking_table.call_type,db.sm_tracking_table.visit_time, orderby=db.sm_tracking_table.visit_time, groupby=db.sm_tracking_table.zone_id)
    # return recFirst_submit
    # ======================= DAILY/MONTHLY DATA =====================
    #------------------filter
    ...
    if btn_filter:
        ...
        # str(session.search_value_client)
        session.zone_search=str(request.vars.zone_search).split('|')[0]
        session.region_search=str(request.vars.region_search).split('|')[0]
        session.area_search=str(request.vars.area_search).split('|')[0]
        session.teri_search=str(request.vars.teri_search).split('|')[0]
        # session.search_value=str(request.vars.search_value).strip().upper()
        reqPage=0
    elif btn_all:
        session.btn_filter=None
        session.search_type=None
        session.zone_search=None
        session.region_search=None
        session.area_search=None
        session.teri_search=None
        reqPage=0
    ...
    qset=qset (db.sm_order_head.order_date ==current_date)
    if session.btn_filter:
        # return 'jhkj'
        if session.zone_search:

            qset=qset(db.sm_order_head.level0_id==session.zone_search)
        if session.region_search:
            qset=qset(db.sm_order_head.level1_id==session.region_search)
        if session.area_search:
            qset=qset(db.sm_order_head.level2_id==session.area_search)
        if session.teri_search:
            qset=qset(db.sm_order_head.level3_id==session.teri_search)

    sell_count=''
    rec_selldailyDatac=qset.select(db.sm_order_head.id.count(), orderby=db.sm_order_head.id)
    # return rec_selldailyDatac
    for rec_selldailyDatac in rec_selldailyDatac:
        sell_count = rec_selldailyDatac[db.sm_order_head.id.count()]  

    dayCount=(current_date).split('-')[2]
    sell_countavg=str(float(sell_count)/float(dayCount))
    # return sell_countavg
    ...
    qset_d=db()
    qset_d=qset_d(db.sm_doctor_visit.cid == c_id)
    qset_d=qset_d (db.sm_doctor_visit.visit_date ==current_date)
    if session.btn_filter:
        # return 'jhkj'
        if session.zone_search:

            qset_d=qset_d(db.sm_doctor_visit.level0_id==session.zone_search)
        if session.region_search:
            qset_d=qset_d(db.sm_doctor_visit.level1_id==session.region_search)
        if session.area_search:
            qset_d=qset_d(db.sm_doctor_visit.level2_id==session.area_search)
        if session.teri_search:
            qset_d=qset_d(db.sm_doctor_visit.level3_id==session.teri_search)

    doc_count=''
    rec_DocdailyDatac=qset_d.select(db.sm_doctor_visit.doc_id.count(), orderby=db.sm_doctor_visit.doc_id)
    for rec_DocdailyDatac in rec_DocdailyDatac:
        doc_count = rec_DocdailyDatac[db.sm_doctor_visit.doc_id.count()]  
    # return sell_count
    doc_countavg=str(float(doc_count)/float(dayCount))




    qset_ch=db()
    qset_ch=qset_ch(db.sm_tracking_table.cid == c_id)
    qset_ch=qset_ch (db.sm_tracking_table.visit_date ==current_date)
    qset_ch=qset_ch (db.sm_tracking_table.call_type =='CHECKIN')
    if session.btn_filter:
        if session.area_search:
            qset_ch=qset_ch(db.sm_tracking_table.area_id==session.area_search)
       

    checkIn_count=''
    rec_checkIndailyDatac=qset_ch.select(db.sm_tracking_table.id.count(), orderby=db.sm_tracking_table.id)
    for rec_checkIndailyDatac in rec_checkIndailyDatac:
        checkIn_count = rec_checkIndailyDatac[db.sm_tracking_table.id.count()]  
    # return checkIn_count
    checkIn_countavg=str(float(checkIn_count)/float(dayCount))




    qset_f=db()
    qset_f=qset_f(db.sm_farm_visit.cid == c_id)
    qset_f=qset_f((db.sm_farm_visit.created_on>=current_date) or (db.sm_farm_visit.created_on<current_date))
    if session.btn_filter:

        if session.teri_search:
            qset_f=qset_f(db.sm_farm_visit.route==session.teri_search)

    farm_count=''
    rec_farmdailyDatac=qset_f.select(db.sm_farm_visit.id.count(), orderby=db.sm_farm_visit.id)
    for rec_farmdailyDatac in rec_farmdailyDatac:
        farm_count = rec_farmdailyDatac[db.sm_farm_visit.id.count()] 

    # return farm_count
    farm_countavg=str(float(farm_count)/float(dayCount))

    # ======================= Doctor Visit  DATA =====================
    rec_docVisit=db((db.sm_doctor_visit.cid == c_id) & ((db.sm_doctor_visit.visit_date>=current_date))).select(db.sm_doctor_visit.ALL, orderby=db.sm_doctor_visit.id)
    
    


    return dict(recFirst_submit=recFirst_submit,sell_count=sell_count,doc_count=doc_count,checkIn_count=checkIn_count,farm_count=farm_count,sell_countavg=sell_countavg,doc_countavg=doc_countavg,checkIn_countavg=checkIn_countavg,farm_countavg=farm_countavg,rec_docVisit=rec_docVisit)
...
def daily_data():
    ...
    rec_selldailyData=qset.select(db.sm_order_head.ALL, orderby=db.sm_order_head.order_date)

    return dict(rec_selldailyData=rec_selldailyData)
...
def daily_dataDcr():
    ...
    rec_DocDailyData=qset_d.select(db.sm_doctor_visit.ALL, orderby=db.sm_doctor_visit.visit_date)
    
    return dict(rec_DocDailyData=rec_DocDailyData)

def daily_dataCheckIn():
    c_id=session.cid
    ...
    rec_checkIndailyDatac=qset_ch.select(db.sm_tracking_table.ALL, orderby=db.sm_tracking_table.id)
   
    
    return dict(rec_checkIndailyDatac=rec_checkIndailyDatac)
...
def daily_dataFarm():
    ...
    rec_farmdailyData=qset_f.select(db.sm_farm_visit.ALL, orderby=db.sm_farm_visit.created_on)
    
    return dict(rec_farmdailyData=rec_farmdailyData)

def monthly_selldata():
    c_id=session.cid
    qset=db()
    qset=qset(db.sm_order_head.cid == c_id) 
    qset=qset((db.sm_order_head.order_date>= current_date) or (db.sm_order_head.order_date<current_date))
    if session.btn_filter:
        # return 'jhkj'
        if session.zone_search:
        ...
        if session.teri_search:
            qset=qset(db.sm_order_head.level3_id==session.teri_search)
    rec_sellmonthlyData=qset.select(db.sm_order_head.ALL, orderby=db.sm_order_head.order_date)

    return dict(rec_sellmonthlyData=rec_sellmonthlyData)

def monthly_dataDcr():
    c_id=session.cid
    qset=db()
    qset=qset(db.sm_doctor_visit.cid == c_id)
    qset=qset((db.sm_doctor_visit.visit_date>=current_date) or(db.sm_doctor_visit.visit_date<current_date))
    if session.btn_filter:
        # return 'jhkj'
        if session.zone_search:

            qset=qset(db.sm_doctor_visit.level0_id==session.zone_search)
        if session.region_search:
            qset=qset(db.sm_doctor_visit.level1_id==session.region_search)
        if session.area_search:
            qset=qset(db.sm_doctor_visit.level2_id==session.area_search)
        if session.teri_search:
            qset=qset(db.sm_doctor_visit.level3_id==session.teri_search)
    rec_DocMonthlyData=qset.select(db.sm_doctor_visit.ALL, orderby=db.sm_doctor_visit.visit_date)
    # return rec_DocMonthlyData
    return dict(rec_DocMonthlyData=rec_DocMonthlyData)

def monthly_dataCheckIn():
    c_id=session.cid
    qset=db()
    qset=qset(db.sm_tracking_table.cid == c_id)
    qset=qset((db.sm_tracking_table.visit_date>=current_date) or (db.sm_tracking_table.visit_date<current_date))
    qset=qset(db.sm_tracking_table.call_type =='CHECKIN')
    if session.btn_filter:
        if session.area_search:
            qset=qset(db.sm_tracking_table.area_id==session.area_search)

    rec_checkInmonthlyDatac=qset.select(db.sm_tracking_table.ALL, orderby=db.sm_tracking_table.id)
   
    
    return dict(rec_checkInmonthlyDatac=rec_checkInmonthlyDatac)

def monthly_dataFarm():
    c_id=session.cid
    qset=db()
    qset=qset(db.sm_farm_visit.cid == c_id)
    qset=qset((db.sm_farm_visit.created_on>=current_date) or (db.sm_farm_visit.created_on<current_date))
    if session.btn_filter: 
        if session.teri_search:
            qset=qset(db.sm_farm_visit.route==session.teri_search)
    rec_farmmonthlyData=qset.select(db.sm_farm_visit.ALL, orderby=db.sm_farm_visit.created_on)
    


    return dict(rec_farmmonthlyData=rec_farmmonthlyData)
...
#         url = 'http://e.businesssolutionapps.com/cpanel/get_depot/get_depot?depot_data=' + request_text
        url = 'http://w05.yeapps.com/cpanel/get_depot/get_depot?depot_data=' + request_text
...
#============= farm List =====30-01-19 Rima start=======
def get_farm_list():
    retStr = ''
    cid = session.cid

    rows = db(db.sm_farm.cid == cid).select(db.sm_farm.farm_id, db.sm_farm.farm_name, groupby=db.sm_farm.farm_id,orderby=db.sm_farm.farm_id)

    for row in rows:
        id = str(row.farm_id)
        name = str(row.farm_name).replace('|', ' ').replace(',', ' ')
        if retStr == '':
            retStr = name + '|' + id
        else:
            retStr += ',' + name + '|' + id
...
# =========================== dashboard Zone,Region,area,teri =============

def get_zone_list():
    retStr = ''
    cid = session.cid
    rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 0)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
    for row in rows:
        level_id = str(row.level_id)
        name = str(row.level_name).replace('|', ' ').replace(',', ' ')
...
def get_region_list():
    retStr = ''
    cid = session.cid    
    zoneIdval = request.vars.zoneval
    if not(zoneIdval=='' or zoneIdval==None):
        rows = db((db.sm_level.cid == cid)&(db.sm_level.parent_level_id == zoneIdval)&(db.sm_level.depth == 1)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
    else:
        rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 1)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
...
def get_area_list():
    retStr = ''
    cid = session.cid    
    regionIdval = request.vars.regionValue
    if not(regionIdval=='' or regionIdval==None):
        rows = db((db.sm_level.cid == cid)&(db.sm_level.parent_level_id == regionIdval)&(db.sm_level.depth == 2)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
    else:
        rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 2)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
...
def get_teri_list():
    retStr = ''
    cid = session.cid    
    areaIdval = request.vars.areaValue
    # return areaIdval
    if not(areaIdval=='' or areaIdval==None):
        rows = db((db.sm_level.cid == cid)&(db.sm_level.parent_level_id == areaIdval)&(db.sm_level.depth == 3)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)
    else:
        rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 3)).select(db.sm_level.level_id, db.sm_level.level_name, orderby=db.sm_level.level_name)

...
def tourSubmit():
    ...
    if errorFlag==0:
        ...
        if user_type=='rep':                             
            insStr="update sm_doctor_visit_plan p,sm_microunion m set p.route_name=m.microunion_name, p.level3_id=m.area_id,p.level3_name=m.area_name where m.cid=p.cid and m.microunion_id = p.route_id"                    
            insRun=db.executesql(insStr)               
        else:
            insStrSup="update sm_doctor_visit_plan p,sm_level l set p.route_name=l.level_name,p.level3_id=l.level_id,p.level3_name=l.level_name where l.cid=p.cid and l.level_id = p.route_id"                 
...
def diary_image():
    response.title = 'Diary Image'

    cid = request.vars.cid 
    rep_id = request.vars.memId_1  


    check_Rows=db((db.sm_rep.cid==cid) & (db.sm_rep.rep_id==rep_id) & (db.sm_rep.status=='ACTIVE')).select(db.sm_rep.rep_id,db.sm_rep.name,db.sm_rep.user_type,limitby=(0,1))
    
    
    if check_Rows:
        rep_id=check_Rows[0].rep_id
        diaryImgRows = db((db.sm_diary_image.cid == cid) &(db.sm_diary_image.rep_id == rep_id) ).select(db.sm_diary_image.ALL, orderby=~db.sm_diary_image.id,limitby=(0,10))
        

        return dict(diaryImgRows=diaryImgRows)
    else:

        return dict()

# =========== 20211227
def dImg_sessionUser():
    response.title = 'Diary Image'

    cid = request.vars.cid 
    rep_id = request.vars.user_id  
...
#             return depthNo
        ...
        for levelRow in levelRows:
            ...
            pendingfo = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.level3_id.belongs(areaList)) & (db.sm_doctor_visit_plan.status=='CReq') &(db.sm_doctor_visit_plan.schedule_date> current_date)).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.status, orderby=db.sm_doctor_visit_plan.schedule_date, groupby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.schedule_date)                 
...
def tourPending():
    ...
    pendinginfo = db((db.sm_doctor_visit_plan.cid==session.cid)  &(db.sm_doctor_visit_plan.rep_id==rep_id) & (db.sm_doctor_visit_plan.status=='CReq') & (db.sm_doctor_visit_plan.schedule_date== s_date)).select(db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.note,db.sm_doctor_visit_plan.status, orderby=~db.sm_doctor_visit_plan.note)    
   
    return dict(pendinginfo=pendinginfo,s_date=s_date,rep_id=rep_id)  

...
#         url = 'http://e.businesssolutionapps.com/cpanel/login_permission/check_login?login_data=' + request_text
        url = 'http://w05.yeapps.com/cpanelmrep/login_permission/check_login?login_data=' + request_text
...
#                                if check_role('rm_doctor_visit_manage') == True or check_role('rm_doctor_visit_view') == True:
                                ...
                                #menuDivStr += """<li ><a href="/""" + appName + """/track_mobile/tracking_index?cid="""+session.cid+"""&rep_id=WEB&rep_pass=1234" target="_tblank"><div>Visit Tracking -Map</div></a></li>"""
...
#                                     menuDivStr += """<li ><a href="/""" + appName + """/dashboard/dashboard"><div>Dashboard</div></a></li>"""
                                ...
                                menuDivStr += """<li ><a href="/""" + appName + """/zrpt_tosc/analysis"><div>Analysis 2</div></a></li>"""
...
# =============== FILTER =================
    ...
    qset_ftr=db()
    qset_ftr=qset_ftr(db.sm_tracking_table.cid == c_id) 
    qset_ftr=qset_ftr(db.sm_tracking_table.visit_date == current_date) 

    if session.btn_filter:
        if session.zone_search:
            qset_ftr=qset_ftr(db.sm_tracking_table.zone_id==session.zone_search)

        if session.area_search:
            qset_ftr=qset_ftr(db.sm_tracking_table.area_id==session.area_search)

    recFirst_submit=qset_ftr.select(db.sm_tracking_table.zone_id,db.sm_tracking_table.rep_id,db.sm_tracking_table.rep_name,db.sm_tracking_table.call_type,db.sm_tracking_table.visit_time, orderby=db.sm_tracking_table.visit_time, groupby=db.sm_tracking_table.zone_id)
    ...
    if session.btn_filter:
        if session.zone_search:
            qset_ch=qset_ch(db.sm_tracking_table.zone_id==session.zone_search)
...
#         url = 'http://e.businesssolutionapps.com/cpanel/get_depot/get_depot?depot_data=' + request_text
        url = 'http://w05.yeapps.com/cpanelmrep/get_depot/get_depot?depot_data=' + request_text

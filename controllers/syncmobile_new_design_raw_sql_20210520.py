...
#     return 'SUCCESS<SYNCDATA>'+'Please try later'
    ...
    rep_id_report = str(request.vars.rep_id_report).strip().upper()
    # return rep_id_report
    se_item_report = 'ALL'
    se_market_report = 'ALL'
    
    
    date_from = current_date
    # date_to = str(request.vars.date_to)
...
#        return db._lastsql
        ...
        for i in range(len(records_ov)): 
            ...
            report_str=report_str+'</tr>'#</table>' 
        # date_from=current_date
        # now = datetime.datetime.strptime(current_date, "%Y-%m-%d")
        # date_ton=now - datetime.timedelta(days = 8)
        # date_to=str(date_ton).split(' ')[0]
        
        linkPath="window.open('"+sals_rprt_more_path+"sales_report_detail?"+"cid="+cid+"&rep_id="+rep_id+"&password="+password+"&synccode="+synccode+"&se_item_report="+se_item_report+"&se_market_report="+se_market_report+" ', '_system');"
        # linkPath="window.open('"+sals_rprt_more_path+"sales_report_detail?"+"cid="+cid+"&rep_id="+rep_id+"&password="+password+"&synccode="+synccode+"&date_from="+date_from+"&date_to="+date_to+"&se_item_report="+se_item_report+"&se_market_report="+se_market_report+" ', '_system');"
...
def salesRprt_doctor():
    ...
    sals_rprt_dcr_more_path='http://127.0.0.1:8000/navana/salesDcr_report/'
#    return date_to
...
#  visit Count  
    if (user_type=='REP'):
        ...
        condition=""
        if (se_market_report!="ALL"):        
           condition="and sm_doctor_visit.route_id='"+ str(se_market_report) +"' "
        ...
        linkPath="window.open('"+dcr_rprt_dcr_more_path+"salesDcr_report_detail?"+"cid="+cid+"&rep_id="+rep_id+"&password="+password+"&synccode="+synccode+"&se_item_report="+se_item_report+"&se_market_report="+se_market_report+" ', '_system');"
...
#     return cid
    ...
    compRow = "SELECT cid FROM sm_company_settings WHERE cid='"+cid+"' AND status='ACTIVE' LIMIT 1 "
    # return compRowb
    # compRow = db((db.sm_company_settings.cid == cid) & (db.sm_company_settings.status == 'ACTIVE')).select(db.sm_company_settings.cid, limitby=(0, 1))
    if not compRow:
        return 'FAILED<SYNCDATA>Invalid Company'
...
#                             return 'FAILED<SYNCDATA>You have already synced. Please retry after 30 minutes.'
            ...
            rep_update = "UPDATE sm_rep SET sync_code='"+str(sync_code)+"',first_sync_date='"+str(first_sync_date)+"',last_sync_date='"+str(last_sync_date)+"',sync_count='"+str(sync_count)+"'  WHERE cid='"+cid+"' AND password='"+password+"' AND status='ACTIVE' "
...
# ========================DocCategory & Speciality
            ...
            cat_rows = "SELECT category FROM doc_catagory WHERE cid='"+cid+"' ORDER BY category "
            ...
            spc_rows = "SELECT specialty FROM doc_speciality WHERE cid='"+cid+"' ORDER BY specialty "
...
#                 return db._lastsql
                ...
                for marketRow in marketRows:
                    ...
                    if repareaListn=='':
                        ...
                        repareaListn="'"+str(repareaList).replace("['",""''"")+"'"
                        repareaListn=str(repareaListn).replace("']'","'")
...
#                 ----------Tourplan Market-------------------------------------------              
               ...
               #                ----------------------------Doctor list start-----------------------
                ...
                for marketRow_1 in marketRows:
                    ...
                    doctorRowsn = "SELECT sd.doc_id,sd.doc_name,sda.area_id FROM sm_doctor as sd,sm_doctor_area as sda WHERE sd.cid='"+cid+"' AND sd.status='ACTIVE' AND sda.cid='"+cid+"' AND sda.doc_id=sd.doc_id AND sda.area_id='"+area_id+"' ORDER BY sda.area_id,sd.doc_name "
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'+'<br><br><br>'
            ...
            if noticeRow:
                notice_str="""<table width="500" border="1" cellpadding="0" cellspacing="0" style="border:solid; border-style:solid; border-color:#0CF; color:#fff"> 
                        <tr align="left" class="blackCatHead"  >
                ...
                for noticeRow in noticeRow:  
                    ...
                    notice_str=notice_str+'<tr ><td style="color:#fff">'+str(notice_date)+'</td>'
                    notice_str=notice_str+'<td style="padding-left:0px;color:#fff">'+str(notice)+'</td>'
...
def dmpath():
   ...
   # ====apu
    return '<start>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql/<fd>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql/<fd>http://127.0.0.1:8000/navana/tour_web/<fd>http://w05.yeapps.com/navana/tour_web_members/<end>'
    # =====apu end

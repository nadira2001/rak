        ...
        if not repRow:
           ...
           return retStatus
            ...
            if (user_type == 'rep'):
                ...
                marketRows = db((db.sm_rep_area.cid == cid) & (db.sm_rep_area.rep_id == rep_id)).select(db.sm_rep_area.area_id, db.sm_rep_area.area_name, orderby=db.sm_rep_area.area_name)
...
#                return db._lastsql
                ...
                for clienttCatRows in clienttCatRows:
                    ...
                    if clienttCatStr == '':
                        clienttCatStr = str(cat_type_id) 
                    else:
                        clienttCatStr += '<rd>' + str(cat_type_id) 
                    c=c+1
                if (c==1):
...
#                 ---------------------------------------------------------------------------------------
                ...
                return 'SUCCESS<SYNCDATA>' + str(sync_code) + '<SYNCDATA>' + marketStr + '<SYNCDATA>' + productStr + '<SYNCDATA>' + merchandizingStr + '<SYNCDATA>' + dealerStr + '<SYNCDATA>' + brandStr + '<SYNCDATA>' + complainTypeStr + '<SYNCDATA>' + compFromStr + '<SYNCDATA>' + taskTypeStr + '<SYNCDATA>' + regionStr + '<SYNCDATA>' + giftStr+'<SYNCDATA>'+clienttCatStr
...
#         return db._lastsql
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            if settRows:
                docRouteRows = db((db.sm_doctor_area.cid == cid) & (db.sm_doctor_area.doc_id == doctorID)).select(db.sm_doctor_area.area_id, db.sm_doctor_area.depot_id, db.sm_doctor_area.area_name, limitby=(0, 1))
                if not docRouteRows:
                    errorFlag = 1
...
#                     return giftStr
    ...
    #             return giftStr
        insRes = db.sm_doctor_visit.insert(cid=cid, doc_id=doctorID, doc_name=docName, rep_id=route_id, rep_name=rep_name, feedback=notesPart, ho_status='0', route_id=routeID, depot_id=depotID, visit_dtime=datetime_fixed, visit_date=current_date, giftnsample=itemngiftnsample)

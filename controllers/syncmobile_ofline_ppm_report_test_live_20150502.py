...
#  ==============================   
            ...
            docTourRow=db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id)  & (db.sm_doctor_visit_plan.status == 'Confirmed')  & (db.sm_doctor_visit_plan.schedule_date == today)).select(db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name, orderby=db.sm_doctor_visit_plan.route_name)
#             return docTourRow
            marketStrDoc=''
            for docTourRow in docTourRow:
                ...
                if marketStrDoc == '':
                    marketStrDoc = str(route_id) + '<fd>' + str(route_name)
                else:
                    marketStrDoc += '<rd>' + str(route_id) + '<fd>' + str(route_name)
...
#             Insert Doctor
            ...
            if insertDoctor:  
                 ...
                 db.sm_doctor_area.insert(cid=cid, doc_id=docSL, doc_name=dName,area_id=route,area_name=routeName,address=dCAddress,district=dDist,thana=dThana)
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            if userType=='rep':
                ...
                doctorRows = db((db.sm_doctor.cid == cid) & (db.sm_doctor.status == 'SUBMITTED') & (db.sm_doctor.doc_id == docID) ).update(status='ACTIVE')
...
def tourDocEntry():
    ...
    if not repRow:
         ...
         tour_docList = tour_doc_str.split('<rd>')
         marketList=[]
         marketDict={}
         for i in range(len(tour_docList)):                                                                                                                                                                             
             marketSingle = tour_docList[i]
             if len(marketSingle) > 0:
                 marketRows=db((db.sm_doctor_visit_plan.cid==cid) & (db.sm_doctor_visit_plan.rep_id==rep_id)&(db.sm_doctor_visit_plan.schedule_date==tour_date)&(db.sm_doctor_visit_plan.route_id==marketSingle)).select(db.sm_doctor_visit_plan.route_id,limitby=(0,1))
        #             return db._lastsql
                 if not marketRows:
                     marketDict={'cid':cid,'rep_id':rep_id, 'first_date':firstDate,'schedule_date':tour_date,'route_id':marketSingle,'status':'Submitted'}
    ...
    if not repRow:
       return 'Invalid Authorization'
        ...
        if  userType=='rep':    
            import time
            today= time.strftime("%Y-%m-%d")            
            supRepRow = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id) &  (db.sm_doctor_visit_plan.status == 'Submitted') & (db.sm_doctor_visit_plan.schedule_date >= today)).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name, groupby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.rep_name ,orderby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.rep_name)
    #         return supRepRow
            for supRepRow in supRepRow:
                rep_id=supRepRow.rep_id
                rep_name=supRepRow.rep_name
                rep_id_name=str(rep_name)+'|'+str(rep_id)
                repStr+='<li  style="border-bottom-style:solid; border-color:#CBE4E4;border-bottom-width:thin" > <font id="'+ rep_id +'" onClick="repPendingDoc(\''+rep_id+'\')" class="name" >'+ rep_id_name+'</font></li>'
                
        if  userType=='sup':    
            areaList=[]
            import time
            today= time.strftime("%Y-%m-%d")
            supDepthRow = db((db.sm_supervisor_level.cid == cid) & (db.sm_supervisor_level.sup_id == rep_id) & (db.sm_doctor_visit_plan.status == 'Submitted') & (db.sm_doctor_visit_plan.schedule_date >= today)).select(db.sm_supervisor_level.level_id,db.sm_supervisor_level.level_depth_no ,orderby=db.sm_supervisor_level.level_id)
    #         return supRepRow
...
#                 return level
                today= time.strftime("%Y-%m-%d")  
                supRepRow = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan[level].belongs(areaList)) & (db.sm_doctor_visit_plan.status == 'Submitted') & (db.sm_doctor_visit_plan.schedule_date >= today)).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name, groupby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.rep_name ,orderby=db.sm_doctor_visit_plan.rep_id|db.sm_doctor_visit_plan.rep_name)
        #         return supRepRow
                for supRepRow in supRepRow:
                    rep_id=supRepRow.rep_id
                    rep_name=supRepRow.rep_name
...
def repPendingDoc():
    ...
    if not repRow:
       return 'Invalid Authorization'
        ...
        if  userType=='sup':    
            today= time.strftime("%Y-%m-%d")                   
            recRow = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id_pending) & (db.sm_doctor_visit_plan.status == 'Submitted') & (db.sm_doctor_visit_plan.schedule_date >= today)).select(db.sm_doctor_visit_plan.id,db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date, groupby=db.sm_doctor_visit_plan.route_id|db.sm_doctor_visit_plan.route_name|db.sm_doctor_visit_plan.schedule_date ,orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.route_id|db.sm_doctor_visit_plan.route_name)
    #         return supRepRow
            for recRow in recRow:
                id=str(recRow.id)
                route_id=recRow.route_id
                ...
                mainStr=str(route_name)+'|'+str(route_id)+ ' | '+str(schedule_date)
                submitStr=str(route_id)+ ' | '+str(schedule_date)
                visitPlanMarketComb+='<li  style="border-bottom-style:solid; border-color:#CBE4E4;border-bottom-width:thin" onClick="check_boxTourRouteTrue(\''+id+'\')"> '+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox" onClick="getRouteTour_keyup(\''+id+'\')" name="route_tour'+id+'" value="checkbox" id="route_tour'+id+'"><label for="doc_camp'+id+'"></br></label></td><td  style="text-align:left;" onClick="getRouteTour_keyup(\''+id+'\')">'+'</br><font id="'+ id +'"  class="name" >'+ mainStr+'</font></td></tr>'+'</table>'+'</li>'
...
def tourConfirm_doc():
    ...
    tour_route_str= str(request.vars.tour_route_str).strip()
    ...
    if not repRow:
         ...
         tour_routeList = tour_route_str.split('<rd>')
         marketList=[]
        
         for i in range(len(tour_routeList)):                                                                                                                                                                             
             marketSingle = tour_routeList[i]
             marketList.append(marketSingle)
...
def tourCancel_doc():
    ...
    if not repRow:
         ...
         if len(marketList)>0:
             db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.id.belongs(marketList))).update(status='Cancelled')  
             return 'SUCCESS'
         else:
             return 'FAILED' 
...
def repPendingDocShow():
    ...
    if not repRow:
        ...
        today= time.strftime("%Y-%m-%d")      
        recRow = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id_pending) & (db.sm_doctor_visit_plan.schedule_date >= today)  ).select(db.sm_doctor_visit_plan.id,db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.status, groupby=db.sm_doctor_visit_plan.route_id|db.sm_doctor_visit_plan.route_name|db.sm_doctor_visit_plan.schedule_date ,orderby=db.sm_doctor_visit_plan.schedule_date|db.sm_doctor_visit_plan.route_id|db.sm_doctor_visit_plan.route_name)
#         return db._lastsql
        visitPlanMarketComb='<table width="100%" border="0" cellspacing="0">'
        for recRow in recRow:
            id=str(recRow.id)
            route_id=recRow.route_id
            route_name=recRow.route_name
            ...
            submitStr=str(route_id)+ ' | '+str(schedule_date)
            if (status=='Submitted'):
                visitPlanMarketComb=visitPlanMarketComb+'<tr><td>'+str(schedule_date)+'<td>'+str(route_name)+' | '+str(route_id)+'</td><td><input  type="submit" onClick="tourDelete_doc(\''+id+'\');"   style=" background-color:#09C; color:#FFF; font-size:12px; width:50px;" value=" X "    /></td></tr>'
            if (status=='Confirmed'):
                visitPlanMarketComb=visitPlanMarketComb+'<tr style="background-color:#E6FFF2"><td>'+str(schedule_date)+'<td>'+str(route_name)+' | '+str(route_id)+'</td><td>&nbsp;</td></tr>'
            if (status=='Cancelled'):
                visitPlanMarketComb=visitPlanMarketComb+'<tr style="background-color:#FAEFED"><td>'+str(schedule_date)+'<td>'+str(route_name)+' | '+str(route_id)+'</td><td>&nbsp;</td></tr>'
...
#============================= Test dynamic path
def dmpath():
#     return '<start>http://127.0.0.1:8000/demo/syncmobile_ofline_ppm_report_test_live_20150502/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<fd>http://127.0.0.1:8000/demo/syncmobile_ofline_ppm_report_test_live_20150502/<end>'
    return '<start>http://c003.cloudapp.net/demo/syncmobile_ofline_ppm_report_test_live_20150502/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/syncmobile_ofline_ppm_report_test/<fd>http://c003.cloudapp.net/demo/syncmobile_ofline_ppm_report_test_live_20150502/<end>'

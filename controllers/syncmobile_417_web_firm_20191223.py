            ...
            if m_check_in_Info_select:
                ...
                if m_o_settings_rows:
                    ...
                    if ((float(submit_time) >= float(m_submit_time_s_value)) and (float(submit_time) <= float(m_o_submit_time_s_value))):
                        ...
                        insert_track_check_in = db.sm_tracking_table.insert(cid=cid, rep_id=rep_id, rep_name=sm_rep_name,call_type='morning_check_in',visit_time=submit_date_time,location_detail=lat_long)
            ...
            if m_o_settings_rows:
                ...
                if ((float(submit_time) >= float(m_submit_time_s_value)) and ( float(submit_time) <= float(m_o_submit_time_s_value))):
                    ...
                    if m_check_in_Info_select:
                        ...
                        # Nazma Azam 2019-12-21 start
                        insert_track_check_out = db.sm_tracking_table.insert(cid=cid, rep_id=rep_id, rep_name=sm_rep_name,call_type='morning_check_out',visit_time=submit_date_time,location_detail=lat_long)
                ...
                if m_check_in_Info_select:
                    ...
                    if not m_check_in_Info_select_in:
                        insert_e_check_in = db.sm_attendance.insert(cid=cid, rep_id=rep_id,rep_name=sm_rep_name, user_type=user_type,check_in_date=submit_date, e_check_in=submit_date_time,e_check_in_latlong=lat_long)
                         ...
                         # Nazma Azam 2019-12-21 start
                        insert_track_check_in = db.sm_tracking_table.insert(cid=cid, rep_id=rep_id, rep_name=sm_rep_name, call_type='evening_check_in',visit_time=submit_date_time,location_detail=lat_long)
        ...
        if not repRow:
            ...
            if ((float(submit_time) >= float(e_submit_time_s_value)) and (float(submit_time) <= float(e_o_submit_time_s_value))):
                ...
                if m_check_in_Info_select:
                    ...
                    # Nazma Azam 2019-12-21 start
                    insert_track_check_out = db.sm_tracking_table.insert(cid=cid, rep_id=rep_id, rep_name=sm_rep_name,call_type='evening_check_out',visit_time=submit_date_time,location_detail=lat_long)
...
def fv_submit():
    ...
    if not compRow:
        ...
        if not repRow:
            ...
            insertFarm = db.sm_farm_visit.insert(cid=cid,farm_id=farm_Id, farm_name=farm_name, route=route, latitude=latitude,longitude=longitude, farm_type=farm_type, meet_with=tr_owner, visit_type=tr_visitType, doc_support=tr_doctor, rx_value=tr_rx, note=tr_note)

...
#        return db._lastsql
    ...
    if (user_type=='SUP'):
        ...
        for i in range(len(records_ov)): 
            ...
            qset_vc_str="SELECT count(sl) as Vcount FROM sm_order_head WHERE cid =  '"+ str(cid) +"' AND delivery_date >=  '"+ str(date_from) +"' AND delivery_date <  '"+ str(date_to) +"' AND area_id = '"+str(area_id)+"'" 
            reportRows_count=db.executesql(qset_vc_str,as_dict = True)
            # return  qset_vc_str
            visit_count=0
            for reportRows_count in reportRows_count:
            ...
            qset_oc_str="SELECT count(sl) as Ocount FROM sm_order_head WHERE cid =  '"+ str(cid) +"' AND field1 = 'ORDER' AND delivery_date >=  '"+ str(date_from) +"' AND delivery_date <  '"+ str(date_to) +"' AND area_id = '"+str(area_id)+"'" 
            reportRows_order_count=db.executesql(qset_oc_str,as_dict = True)
...
# ============================================   
            ...
            productcomRows = "SELECT offer_id,offer_name FROM  sm_doctor_offer WHERE cid='"+cid+"' ORDER BY offer_id"
            ...
            for productcomRow in productcomRow:
                item_id = str(productcomRow['offer_id'])
                name = str(productcomRow['offer_name'])
            ...
            # =================offer============
            # productOfferRows = "SELECT item_id,name FROM sm_item WHERE cid='"+cid+"' ORDER BY item_id limit 5"
            # productOfferRow=db.executesql(productOfferRows,as_dict=True)

            # # productcomRow=db(db.sm_item.cid == cid).select(db.sm_item.item_id, db.sm_item.name)    

            # productOfferStr='' 
            # for productOfferRow in productOfferRow:
            #     item_id_offer = str(productOfferRow['item_id'])
            #     name_offer = str(productOfferRow['name'])
              
            #     if productOfferStr == '':
            #         productOfferStr = str(item_id_offer) + '<fd>' + str(name_offer)
            #     else:
            #         productOfferStr += '<rd>' + str(item_id_offer) + '<fd>' + str(name_offer) 
...
def doctor_visit_submit_pharma():
    ...
    start_time=str(request.vars.start_time).strip()
    if ((start_time==None) or (start_time=='None') or (start_time=='') or (start_time=='undefined')):
        start_time=datetime_fixed
...
def doc_add_submit():
    return 'SUCCESS<SYNCDATA>'+'Restricted'
    cid = str(request.vars.cid).strip().upper()
    rep_id = str(request.vars.rep_id).strip().upper()
    password = str(request.vars.rep_pass).strip()
    synccode = str(request.vars.synccode).strip()
...
def dmpath():
   ...
   # ====apu
    return '<start>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql_test/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql_test/<fd>http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql_test/<fd>http://127.0.0.1:8000/navana/tour_web/<fd>http://w05.yeapps.com/navana/tour_web_members/<end>'
    # =====apu end

                            ...
                            if batchIdrows:
                                ...
                                if not autDelCronRows:
                                    ...
                                    for j in range(len(slList)):
                                        ...
                                        #-------------------------------- Loop start
                                        headRow=db((db.sm_invoice_head.cid==c_id)& (db.sm_invoice_head.depot_id==req_depot) & (db.sm_invoice_head.sl==req_sl)& (db.sm_invoice_head.status=='Submitted')).select(db.sm_invoice_head.id,db.sm_invoice_head.store_id,db.sm_invoice_head.rep_id,db.sm_invoice_head.client_id,db.sm_invoice_head.area_id,db.sm_invoice_head.discount,limitby=(0,1))
                                        if not headRow:

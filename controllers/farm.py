...
#             qset = qset(db.sm_farm.farm_id == search_value_id)
        ...
        elif (session.search_type == 'routeName'):
            searchValue=str(session.search_value).split('|')[0]
            qset = qset(db.sm_farm.route == searchValue)
...
def farm_visit_detail():
    ...
    if (session.btn_filter_farm):
        ...
        if (session.search_type== 'FarmName'):
            ...
            search_value=str(session.search_value).split('|')[0]
            qset = qset(db.sm_farm_visit.route == search_value)
...
def get_Vroute_list():
    ...
    records = db(db.sm_farm_visit.cid == cid).select(db.sm_farm_visit.route, groupby=db.sm_farm_visit.route,orderby=db.sm_farm_visit.route)

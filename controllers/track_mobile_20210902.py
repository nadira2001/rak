...
def tracking_index():
    ...
    if rep_id=='WEB':
        ...
        for repRecords in repRecords:
            ...
            dictData={'u_id':rep_id,'u_name':name}
        ...
        if not userRecords:
            ...
            if user_type=='rep':
                ...
                repRows=db((db.sm_rep_area.cid==c_id) & (db.sm_rep_area.area_id.belongs(area_list))).select(db.sm_rep_area.rep_id,db.sm_rep_area.rep_name)
                # return repRows
                for row1 in repRows:
                ...
                if not supLevelRows:
                    ...
                    if level_depth_no==0:
                        ...
                        supRows=db((db.sm_supervisor_level.cid==c_id) & (db.sm_supervisor_level.level_id.belongs(level1_list))).select(db.sm_supervisor_level.sup_id,db.sm_supervisor_level.sup_name)
                        for row in supRows:
                        ...
                        supRows=db((db.sm_supervisor_level.cid==c_id) & (db.sm_supervisor_level.level_id.belongs(level2_list))).select(db.sm_supervisor_level.sup_id,db.sm_supervisor_level.sup_name)
                        for row in supRows:
                            sup_id=str(row.sup_id)
                            sup_name=str(row.sup_name)
                        ...
                        repRows=db((db.sm_rep_area.cid==c_id) & (db.sm_rep_area.area_id.belongs(level3_list))).select(db.sm_rep_area.rep_id,db.sm_rep_area.rep_name)
                        for row1 in repRows:
                            rep_id=str(row1.rep_id)
                            rep_name=str(row1.rep_name)
                            
                            dictData={'u_id':rep_id,'u_name':rep_name}
                            level_user_list.append(dictData)

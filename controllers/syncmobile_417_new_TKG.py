...
def fileUploader_docVisit():
    ...
    file = request.vars.upload.file
#    shutil.copyfileobj(file,open('C:/workspace/Dropbox/sabbir_acer/web2py/applications/welcome/uploads/'+filename,'wb'))
    shutil.copyfileobj(file, open('/home/www-data/web2py/applications/tkg_image/static/tkg_pic/' + filename, 'wb'))
    return 'success'
...
#     return 'FAILED<SYNCDATA>Please Try Later.'
    ...
    if not compRow:
        ...
        repRow = db((db.sm_rep.cid == cid) & (db.sm_rep.rep_id == rep_id) & (db.sm_rep.password == password) & (db.sm_rep.status == 'ACTIVE')).select(db.sm_rep.id, db.sm_rep.name, db.sm_rep.sync_code, db.sm_rep.sync_count, db.sm_rep.first_sync_date, db.sm_rep.last_sync_date, db.sm_rep.user_type, db.sm_rep.depot_id, db.sm_rep.level_id, db.sm_rep.field2, limitby=(0, 1))
...
# ==================================
#             itemCatRecords=db((db.sm_category_type.cid==cid)& (db.sm_category_type.type_name=='ITEM_CATEGORY')).select(db.sm_category_type.ALL,orderby=db.sm_category_type.cat_type_id)
#             catNameList = ''
#             for itemRecord in itemCatRecords:
#                 catName = itemRecord.cat_type_id
#                 
#                 if catNameList=='':
#                     catNameList = catName
#                 else:
#                     catNameList +='<fd>'+ catName
                    
            catNameList = 'ATTA<fd>MAIDA<fd>MO<fd>RBO<fd>RPO<fd>RSO<fd>SUJI<fd>TEA'   
            clTypeStr = 'Grocery<fd>DepartmentalStore<fd>SuperStore<fd>HTS<fd>Inst<fd>Others'  
            clientCatList = 'Oil<fd>Flour<fd>Tea<fd>MOil<fd>All'  
            
# ==========Client Category and sub category            
            clCatStr = ''
            rows_clCat=db((db.sm_category_type.cid==cid) & (db.sm_category_type.type_name=='CLIENT_CATEGORY') ).select(db.sm_category_type.cat_type_id,db.sm_category_type.cat_type_name)
            for rows_clCat in rows_clCat:
                if clCatStr == '':
                    clCatStr = str(rows_clCat.cat_type_id) + '|' + str(rows_clCat.cat_type_name)
                else:
                    clCatStr += '<rd>' + str(rows_clCat.cat_type_id) + '|' + str(rows_clCat.cat_type_name)
            clCatStr += '<rd>'
            clCatSubStr = ''
            rows_clsubCat=db((db.sm_category_type.cid==cid) & (db.sm_category_type.type_name=='CLIENT_SUB_CATEGORY') ).select(db.sm_category_type.cat_type_id,db.sm_category_type.cat_type_name)
            for rows_clsubCat in rows_clsubCat:
                if clCatSubStr == '':
                    clCatSubStr = str(rows_clsubCat.cat_type_id) + '|' + str(rows_clsubCat.cat_type_name)
                else:
                    clCatSubStr += '<rd>' + str(rows_clsubCat.cat_type_id) + '|' + str(rows_clsubCat.cat_type_name)
            clCatSubStr += '<rd>'  
            if (user_type == 'rep'):
...
#                 marketTourRows = db((db.sm_rep_area.cid == db.sm_depot_market.cid) &(db.sm_doctor_area.cid == db.sm_depot_market.cid)&(db.sm_doctor_area.area_id == db.sm_rep_area.area_id) & (db.sm_rep_area.rep_id == rep_id) & (db.sm_doctor_area.field1 == db.sm_depot_market.market_id)).select(db.sm_depot_market.market_id, db.sm_depot_market.market_name, orderby=db.sm_depot_market.market_name, groupby=db.sm_depot_market.market_id)
                marketTourRows = db((db.sm_doctor_area.cid==cid) & (db.sm_doctor_area.field1!='') & (db.sm_doctor_area.note!='')  &(db.sm_doctor_area.area_id.belongs(repareaList))).select(db.sm_doctor_area.field1,db.sm_doctor_area.note, orderby=db.sm_doctor_area.note, groupby=db.sm_doctor_area.field1)
#                 return db._lastsql
...
#                 return first_currentDate
                ...
                marketTourStr=''
#                 marketTourRows = db((db.sm_rep_area.cid == db.sm_depot_market.cid) &(db.sm_doctor_area.cid == db.sm_depot_market.cid)&(db.sm_doctor_area.area_id == db.sm_rep_area.area_id) & (db.sm_rep_area.rep_id == rep_id) & (db.sm_doctor_area.field1 == db.sm_depot_market.market_id)).select(db.sm_depot_market.market_id, db.sm_depot_market.market_name, orderby=db.sm_depot_market.market_name, groupby=db.sm_depot_market.market_id)
                marketTourRows = db((db.sm_doctor_area.cid==cid) & (db.sm_doctor_area.field1!='') & (db.sm_doctor_area.note!='') &(db.sm_doctor_area.area_id.belongs(marketList))).select(db.sm_doctor_area.field1,db.sm_doctor_area.note, orderby=db.sm_doctor_area.note, groupby=db.sm_doctor_area.field1)
#                 return db._lastsql
                for marketTourRow in marketTourRows:
                    market_id =  marketTourRow.field1
                    market_name =  marketTourRow.note
...
#     
        ...
        repRow = db((db.sm_rep.cid == cid) & (db.sm_rep.rep_id == rep_id) & (db.sm_rep.password == password) & (db.sm_rep.sync_code == synccode) & (db.sm_rep.status == 'ACTIVE')).select(db.sm_rep.ALL, limitby=(0, 1)) #db.sm_rep.rep_id, db.sm_rep.name, db.sm_rep.mobile_no, db.sm_rep.user_type, db.sm_rep.level_id
#         return cid+', '+rep_id+', '+password+', '+synccode
...
#             here is the problem may be ################################################
            ...
            if not clientRecords:
                ...
                depotRow = db((db.sm_depot.cid == cid)).select(db.sm_depot.id, db.sm_depot.name, db.sm_depot.order_sl, limitby=(0, 1))
...
#                 inbox_str=inbox_str+'<tr height="30px"><td ><font style="font-size:14px;color:#339">'+str(msgTime)+'</font></br><font style="font-size:12px;color:#633">From: '+str(msgFromInfo)+'</font></br><font style="font-size:16px;color:#366;font-style:italic">'+str(msg)+'</font></td></tr>'
                inbox_str=inbox_str+'<li height="30px"><font style="font-size:11px;color:#339; font-weight:bold">'+str(msgTime)+'</font></br><font style="font-size:11px;color:#633; font-weight:bold">From: '+str(msgFromInfo)+'</font></br><font style="font-size:15px;color:#366; margin-top:3px">'+str(msg)+'</font></li>'
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'+'<br><br><br>'
            ...
            records=qset.select(db.sm_order.ALL,orderby=~db.sm_order.sl, limitby=(0,3))
...
#                         <td width="50" align="right"  >Amount </td>
#                         </tr>"""
                    invoice_str=invoice_str+"""<table width="300" border="1" cellpadding="0" cellspacing="0" style="border:solid; border-style:solid; border-color:#0CF; color:#306161"> 
                        <tr align="left" class="blackCatHead"  >
...
#                     invoice_str=invoice_str+'<td ></td>'
                    ...
                    invoice_str=invoice_str+'</tr></table><br>'       
                    sl_Past=str(record.sl)
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'+'<br><br><br>'
            ...
            return 'SUCCESS<SYNCDATA>'+ db._lastsql
            if noticeRow:
...
def chemist_submit():
    ...
    chemist_address = str(request.vars.chemist_add).strip().decode("ascii", "ignore")
    chemist_ph = str(request.vars.chemist_ph).strip().decode("ascii", "ignore")
    managerName = str(request.vars.managerName).strip().decode("ascii", "ignore")
    managerContactNumber = str(request.vars.managerContactNumber).strip().decode("ascii", "ignore")
    propiterName = str(request.vars.propiterName).strip().decode("ascii", "ignore")
    clTypeL = str(request.vars.clTypeL).strip().decode("ascii", "ignore")
    clientCatL = str(request.vars.clientCatL).strip().decode("ascii", "ignore")
    ...
    clClient = str(request.vars.clClient).strip().decode("ascii", "ignore")
    clSubClient = str(request.vars.clSubClient).strip().decode("ascii", "ignore")
    if clClient!='' and clClient!=None:
        clClientID=clClient.split('|')[0]
        clClientName=clClient.split('|')[1]
    if clSubClient!='' and clSubClient!=None:
        clSubClientID=clSubClient.split('|')[0]
        clSubClientName=clSubClient.split('|')[1]
...
#         return db._lastsql
        if not repRow:
           ...
           return retStatus
            ...
            slRows= db(db.sm_client.cid == cid).select(db.sm_client.client_id, orderby=~db.sm_client.id, limitby=(0, 1))
#             slRow= db((db.sm_settings_pharma.cid == cid) & (db.sm_settings_pharma.s_key == 'CHEMSL')).select(db.sm_settings_pharma.s_value, orderby=~db.sm_settings_pharma.s_value, limitby=(0, 1))
            ...
            clientId=0
            for slRow in slRows:
                clId = slRow.client_id
            
            clientId= int(clId)
            maxClient_id=clientId+1
            
            clientRows= db((db.sm_client.cid == cid) & (db.sm_client.area_id == market_id) & (db.sm_client.depot_id != '')).select(db.sm_client.depot_id,db.sm_client.depot_name,db.sm_client.store_id,db.sm_client.store_name, orderby=~db.sm_client.id, limitby=(0, 1))
...
#             slRow= db((db.sm_settings_pharma.cid == cid) & (db.sm_settings_pharma.s_key == 'CHEMSL')).select(db.sm_settings_pharma.s_value, orderby=~db.sm_settings_pharma.s_value, limitby=(0, 1))
            ...
            for clientRows in clientRows:
                depot_id=clientRows.depot_id
                depot_name=clientRows.depot_name
                store_id=clientRows.store_id
                store_name=clientRows.store_name
            
            
            client_insert=db.sm_client.insert(cid=cid,client_id=maxClient_id,name=chemist_name, area_id=market_id,contact_no1= chemist_ph , manager_name=managerName, address = chemist_address,  manager_contact_no=managerContactNumber , created_by= rep_id,  dob= chemist_dob,status='Submitted',    field1=imageName,depot_id=depot_id,depot_name=depot_name,store_id=store_id,store_name=store_name,owner_name=propiterName,category_id=clClientID,category_name=clClientName, sub_category_id=clSubClientID,sub_category_name=clSubClientName,kids_info=clTypeL,note=clientCatL)            
...
#-----------------------------pendingClientApproval start
def pendingClientApproval():
    cid = str(request.vars.cid).strip().upper()
    area_id = str(request.vars.area_id).strip().upper()
    
    compRows = db((db.sm_client.cid == cid) & (db.sm_client.status == 'Submitted')& (db.sm_client.area_id == area_id)).select(db.sm_client.id, db.sm_client.name, db.sm_client.address, db.sm_client.contact_no1, db.sm_client.manager_name, db.sm_client.manager_contact_no, db.sm_client.area_id, db.sm_client.field1, db.sm_client.category_id, db.sm_client.category_name, db.sm_client.sub_category_id, db.sm_client.sub_category_name, db.sm_client.owner_name, orderby=~db.sm_client.id)
    retStr=''
    for compRow in compRows:
        clientId = compRow.id
        clientName = compRow.name
        clientAddress = compRow.address
        clienContactNo = compRow.contact_no1
        clientManagerName = compRow.manager_name
        clientManagerContactNo = compRow.manager_contact_no
        clientArea = compRow.area_id
        imageName = compRow.field1
        category_id= compRow.category_id
        category_name= compRow.category_name
        sub_category_id= compRow.sub_category_id
        sub_category_name= compRow.sub_category_name
        owner_name=compRow.owner_name
        
        
        if retStr=='':
            retStr = clientName + '<fd>' + clientAddress + '<fd>' + str(clienContactNo) + '<fd>' + clientManagerName + '<fd>' + str(clientManagerContactNo)+ '<fd>' + str(clientId)+ '<fd>' + clientArea+'<fd>'+category_id + '<fd>' + category_name + '<fd>' + sub_category_id + '<fd>' + sub_category_name+'<fd>'+owner_name
        else:
            retStr += '<rd>' + clientName + '<fd>' + clientAddress + '<fd>' + str(clienContactNo) + '<fd>' + clientManagerName + '<fd>' + str(clientManagerContactNo)+ '<fd>' + str(clientId)+ '<fd>' + clientArea+'<fd>'+category_id + '<fd>' + category_name + '<fd>' + sub_category_id + '<fd>' + sub_category_name+'<fd>'+owner_name
        
    return retStr

#-----------------------------pendingClientApprovalUpdate start
def pendingClientApprovalUpdate():
    cid = str(request.vars.cid).strip().upper()
    clientId = str(request.vars.client_id).strip().upper()
    clName = str(request.vars.clName).strip().upper()
    clAdd = str(request.vars.clAdd).strip().upper()
    clPphone = request.vars.clPphone
    clMname = str(request.vars.clMname).strip().upper()
    clMphone = request.vars.clMphone
    ownerName = request.vars.ownerName
    clCat = request.vars.clCat
    clSubCat = request.vars.clSubCat
    ...
    if clCat!='' and clCat!=None:
        clClientID=clCat.split('|')[0]
        clClientName=clCat.split('|')[1]
    if clSubCat!='' and clSubCat!=None:
        clSubClientID=clSubCat.split('|')[0]
        clSubClientName=clSubCat.split('|')[1]
#     return clMphone
    
    
    
    approveVal = str(request.vars.sVal).strip().upper()
    updateRows = ''
    if approveVal == 'APPROVE':
        updateRows = db((db.sm_client.cid == cid) & (db.sm_client.id == clientId)).update(name=clName,address=clAdd,contact_no1=clPphone,manager_name=clMname,manager_contact_no=clMphone,status='ACTIVE',category_id=clClientID,category_name=clClientName,sub_category_id=clSubClientID,sub_category_name=clSubClientName)
        return 'APPROVED'
    else:
        updateRows = db((db.sm_client.cid == cid) & (db.sm_client.id == clientId)).update(status='CANCEL')
        return 'CANCELD'
...
#         return db._lastsql
        ...
        microunionRow=db((db. sm_doctor_area.cid==cid) & (db. sm_doctor_area.area_id == route)  & (db.sm_doctor_area.field1 != '')  & (db.sm_doctor_area.note != '')).select(db.sm_doctor_area.field1,db.sm_doctor_area.note, groupby=db.sm_doctor_area.field1, orderby=db.sm_doctor_area.note)
        microunionStr=''
...
#============================= Test dynamic path
def dmpath():
#     return '<start>http://w02.yeapps.com/tkg/syncmobile_417_new/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://104.155.225.205/gpl_image/syncmobile_prescription/<fd>http://w02.yeapps.com/tkg/syncmobile_417_new/<end>'
#     return '<start>http://a007.yeapps.com/acme/syncmobile_417_new/<fd>http://a007.yeapps.com/acme/static/<fd>http://a007.yeapps.com/acme/syncmobile_417_new/<fd>http://a007.yeapps.com/acme/syncmobile_417_new/<end>'
    return '<start>http://127.0.0.1:8000/demo/syncmobile_417_new_TKG/<fd>http://e2.businesssolutionapps.com/mrepbiopharma/static/<fd>http://104.155.225.205/gpl_image/syncmobile_prescription/<fd>http://127.0.0.1:8000/tkg/syncmobile_417_new/<end>'

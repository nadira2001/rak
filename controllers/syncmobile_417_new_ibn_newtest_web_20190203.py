...
#                 Nazma Akhter 2019-01-19 start
            ...
            if m_check_in_Info_select:                                        
                return 'FAILED<SYNCDATA>Already Morning Check In Exists !'

            else:
                settings_rows= db((db.sm_settings.cid == cid) & (db.sm_settings.s_key == 'm_check_in') ).select(db.sm_settings.ALL, limitby=(0,1))
                if settings_rows:
                    submit_time_s_value = settings_rows[0].s_value
                    
                    if (float(submit_time)<=float(submit_time_s_value)):                                                        
                                ...
                                if insert_m_check_in:
                                    return 'SUCCESS<SYNCDATA>Morning Check In Successfully !'
                    else:
                        return 'FAILED<SYNCDATA>Sorry You Are Late !'
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            m_settings_rows= db((db.sm_settings.cid == cid) & (db.sm_settings.s_key == 'm_check_out') ).select(db.sm_settings.ALL, limitby=(0,1))
            if m_settings_rows:
...
#             return str(submit_time)+'--'+str(m_submit_time_s_value)+'--'+str(e_submit_time_s_value)
            if ((float(submit_time)>float(m_submit_time_s_value)) and (float(submit_time)<=float(e_submit_time_s_value)) ):
...
#                 stest=db((db.sm_attendance.cid == cid) & (db.sm_attendance.rep_id == rep_id) & (db.sm_attendance.check_in_date == submit_date)   & (db.sm_attendance.m_check_in_latlong != '0,0')   & (db.sm_attendance.m_check_out_latlong != '0,0') & (db.sm_attendance.e_check_in_latlong == '0,0') ).select(db.sm_attendance.ALL)
#                 return stest
                if m_check_in_Info_select:
                        return 'SUCCESS<SYNCDATA>Evening Check In Successfully !'
...
#                     return m_check_in_Info_select_in
#                     return str(cid)+'--'+str(rep_id)+'--'+str(submit_date)
                    if not m_check_in_Info_select_in:
                        insert_e_check_in = db.sm_attendance.insert(cid=cid, rep_id=rep_id, user_type=user_type, check_in_date=submit_date, e_check_in=submit_date_time, e_check_in_latlong=lat_long)  
                        if insert_e_check_in:
                            return 'SUCCESS<SYNCDATA>Evening Check In Successfully !'

                    else:
                        return 'SUCCESS<SYNCDATA>You Are Not Allowed For Evening Check In !  1'
                    
                    
                
            else:
                return 'SUCCESS<SYNCDATA>You Are Not Allowed For Evening Check In !  2'
...
#                     return db._lastsql
                    if m_check_in_Info_select:                    
                        ...
                        if m_check_out_Info:
                            return 'SUCCESS<SYNCDATA>Morning Check Out Successfully !'
                    else:
                        return 'SUCCESS<SYNCDATA>You Are Not Allowed For Morning Check Out !'
...
#                 return db._lastsql
                if m_check_in_Info_select:        
                        ...
                        if e_check_out_Info:
                            return 'SUCCESS<SYNCDATA>Evening Check Out Successfully !'

                else:
                    return 'SUCCESS<SYNCDATA>You Are Not Allowed For Evening Check Out !'
...
#         ================SL ready============
        ...
        farmInsert = db.sm_farm.insert(cid=cid, farm_id=farm_id, farm_name=addPName,farm_type='POULTRY' ,latitude=latitude, longitude=longitude, owner_name=addPOName , address=AddressP, medicine=chemist_medicine_p, manger_name=managerP,consultant_name=consultantP, category=addCCategoryP, birds_animal=birds_animal_p, rearing_housing=rearingP, feeding=feedingP, watering=wateringP ,brooding=broodingP , poandsSize=addPondsP, anniversary=anniversaryP, dob=addCDOBp)
        if farmInsert:
             return 'SUCCESS<SYNCDATA>Submitted successfully!'
        else:
            return 'SUCCESS<SYNCDATA>Sorry ! PLease try again !'
...
#             client_insert=db.sm_farm.insert(cid=cid,farm_id=farm_id,farm_name=farm_name,owner_name=owner_name,address=str(address),dob=dob,anniversary=anniversary,medicine=medicine,manger_name=manger_name,consultant_name=consultant_name,category=category,birds_animal=birds_animal,rearing_housing=rearing_housing,feeding=feeding,watering=watering,brooding=brooding,poandsSize=poandsSize,status='ACTIVE')#,image=imageName,latitude=latitude,longitude=longitude)            
        client_insert=db.sm_farm.insert(cid=cid,farm_id=farm_id,farm_name=farm_name,farm_type='CATTLE',owner_name=owner_name,address=address,dob=dob,anniversary=anniversary,medicine=medicine,manger_name=manger_name,consultant_name=consultant_name,category=category,birds_animal=birds_animal,rearing_housing=rearing_housing,feeding=feeding,watering=watering,brooding=brooding,poandsSize=poandsSize)#,image=imageName,latitude=latitude,longitude=longitude)            
...
# #         ================SL ready============
        ...
        farmInsert = db.sm_farm.insert(cid=cid,farm_name=addAName, farm_id=farm_id,farm_type='AQUA',latitude=latitude, longitude=longitude, owner_name=addAOName , address=AddressA, medicine=chemist_medicine_a, manger_name=managerA,consultant_name=consultantA, category=addCCategoryA, birds_animal=birds_animal_a, rearing_housing=rearingA, feeding=feedingA, watering=wateringA ,brooding=broodingA , poandsSize=addPondsA, anniversary=anniversaryA, dob=addCDOBa)
        if farmInsert:
            return 'SUCCESS<SYNCDATA>Submitted successfully!'
        else:
            return 'SUCCESS<SYNCDATA>Sorry ! PLease try again !'
...
def farmListData():
    ...
    if (status == 'ACTIVE'):
            farmRows = db((db.sm_farm.cid == cid) & (db.sm_farm.status == 'ACTIVE')).select(db.sm_farm.ALL, orderby=db.sm_farm.farm_name)
            farmStr=''
...
#============================= Test dynamic path
def dmpath():
    return '<start>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://a007.yeapps.com/novivo/tour_web/<fd>http://w011.yeapps.com/novivo/tour_web_members/<end>'
    return '<start>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://w011.yeapps.com/novivo/syncmobile_417_new_ibn_newtest_web/<fd>http://w011.yeapps.com/novivo/tour_web/<fd>http://w011.yeapps.com/novivo/tour_web_members/<end>'
    return '<start>http://w05.yeapps.com/mundi/syncmobile_417_new_ibn_newtest_web/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://w05.yeapps.com/mundi/syncmobile_417_new_ibn_newtest_web/<fd>http://w05.yeapps.com/mundi/syncmobile_417_new_ibn_newtest_web/<fd>http://a007.yeapps.com/mundi/tour_web/<fd>http://w05.yeapps.com/mundi/tour_web_members/<end>'
#     return '<start>http://127.0.0.1:8000/kpl/syncmobile_417_new_ibn_newtest_web/<fd>http://i001.yeapps.com/image_hub/static/kpl_image/<fd>http://i001.yeapps.com/image_hub/kpl_image/<fd>http://127.0.0.1:8000/kpl/syncmobile_417_new_ibn_newtest_web/<fd>http://127.0.0.1:8000/kpl/syncmobile_417_new_ibn_newtest_web/<end>'
    return '<start>http://w05.yeapps.com/kpl/syncmobile_417_new_ibn_newtest_web/<fd>http://i001.yeapps.com/image_hub/static/kpl_image/<fd>http://i001.yeapps.com/image_hub/kpl_image/<fd>http://w05.yeapps.com/kpl/syncmobile_417_new_ibn_newtest_web/<fd>http://w05.yeapps.com/kpl/syncmobile_417_new_ibn_newtest_web/<fd>http://a007.yeapps.com/kpl/tour_web/<fd>http://w05.yeapps.com/kpl/tour_web_members/<end>'
    return '<start>http://w02.yeapps.com/vitalac/syncmobile_417_new_ibn_newtest/<fd>http://w02.yeapps.com/vitalac/static/<fd>http://i001.yeapps.com/image_hub/vitalac_image/<fd>http://w02.yeapps.com/vitalac/syncmobile_417_new_ibn_newtest/<fd>http://w02.yeapps.com/vitalac/syncmobile_417_new_ibn_newtest/<end>'
#     return '<start>http://127.0.0.1:8000/demo/syncmobile_417_new_ibn_newtest/<fd>http://w02air.azurewebsites.net/gpl/static<fd>http://104.155.225.205/gpl_image/syncmobile_prescription/<fd>http://127.0.0.1:8000/demo/syncmobile_417_new_ibn/<fd>http://127.0.0.1:8000/demo/syncmobile_417_new_ibn/<end>'
    return '<start>http://w02.yeapps.com/ipi/syncmobile_417_new_ibn_newtest/<fd>http://w02.yeapps.com/ipi/static/<fd>http://i001.yeapps.com/image_hub/ibnsina_image/ibnsina_image/<fd>http://w02.yeapps.com/ipi/syncmobile_417_new_ibn_newtest/<fd>http://a007.yeapps.com/ipi/syncmobile_417_new_ibn/<end>'

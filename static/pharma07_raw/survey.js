...
/******** jahangirEditedEnd16 apipath *****************/
    ...
    $(document).ready(function(){
		...
		$('#productl_combo').empty();
		$('#productl_combo').append(localStorage.visit_product_combo_1);
		
		$('#product2_combo').empty();
		$('#product2_combo').append(localStorage.visit_product_combo_1);
		
		$('#product3_combo').empty();
		$('#product3_combo').append(localStorage.visit_product_combo_1);
		
		
		// 2020-01-16 start
		$("#wait_image_check").hide();
		// 2020-01-16 end
...
function check_in_Submit(){
	//alert ('111')
	$("#wait_image_check").show();
	$("#image_check_in_link").hide();
	...
	//alert (localStorage.base_url+'checkInSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&user_type='+localStorage.user_type+'&synccode='+localStorage.synccode+'&latitude='+latitude+'&longitude='+longitude)	

	$.ajax(localStorage.base_url+'checkInSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&user_type='+localStorage.user_type+'&synccode='+localStorage.synccode+'&latitude='+latitude+'&longitude='+longitude,{
								type: 'POST',
								timeout: 30000,
								error: function(xhr) {
								$("#wait_image_check").hide();
								$("#image_check_in_link").show();
...
function page_saved_Doc() {
	...
	var docSaveStr=''
	docSaveStr=docSaveStr+' <table bordercolor="#009999" style="color:#fff" height="30px" width="100%" border="1" cellpadding="0" cellspacing="0" style="border-radius:5px;">'
	var currentDate = new Date()
	var day = currentDate.getDate();if(parseInt(day)<9)	{day="0" + day};
	var month = currentDate.getMonth() + 1;if(parseInt(month)<9){month="0" +month};
	var year = currentDate.getFullYear()
	...
	for ( i=0; i < docSaveDataListLength-1; i++){	
		...
		if (parseInt(day)==parseInt(day_get)){
			...
			docSaveStr=docSaveStr+' <tr><td style="color:#fff;" height="30px" >'+docShowList[0] +' [Tomorrow]'+'</td><td align="center" style=" border-left:hidden"></td></tr>'
		}
	}
...
function saveDelete_doc(i) {
	...
	for ( i=0; i < docSaveDataListLength-1; i++){	
		...
		//alert (singleDoc)
		//docSaveStr=docSaveStr+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff" > <table width="100%" border="0" cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr><td onClick="saved_Doc_set(\''+i+'\');">'+docShowList[0]+'</td><td width="60px"><input  type="submit" onClick="saveDelete_doc(\''+i+'\');"   style=" background-color:#09C; color:#FFF; font-size:12px; width:50px; ;  " value=" OK "    /></td></tr></table></li>'
		
		docSaveStr=docSaveStr+'<tr><td style="color:#fff" onClick="saved_Doc_set(\''+i+'\');">'+docShowList[0]+'</td></tr>'
	}
	docSaveStr=docSaveStr+'</table>'
	$('#saved_visit_doc').empty()
	$('#saved_visit_doc').append(docSaveStr);				
...
function saved_Doc_set(i) {
	...
	if (localStorage.doc_offer=='YES'){$("#offer_table").show();}		
	if (localStorage.doc_offer=='NO'){$("#offer_table").hide();}		
...
function onError_ready(error) {
	...
	if (localStorage.visit_location_flag=='YES'){
		...
		$("#visit_location_doc").show();
		$("#btn_visit_submit_doc_location").hide();
...
function check_user() {	
	...
	// var  apipath_base_photo_dm='http://127.0.0.1:8000/navana/syncmobile_new_design_raw_sql/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
	
	// var  apipath_base_photo_dm='http://127.0.0.1:8000/navana/syncmobile_new_design_raw_sql/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
	// apu====
	var  apipath_base_photo_dm='http://w05.yeapps.com/navana/syncmobile_new_design_raw_sql_d_start/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
	// apu end====

	// var  apipath_base_photo_dm='http://w05.yeapps.com/dmpath/dmpath_new_design_raw/get_path?CID='+cid +'&HTTPPASS=e99business321cba'
...
//							--------------------------
							...
							localStorage.path_main=path_main
							...
							// alert (localStorage.base_url+'check_user_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&version=12')
							...
							$.ajax(localStorage.base_url+'check_user_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&version=13',{
								// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
								type: 'POST',
								timeout: 30000,
								error: function(xhr) {
								...
								success:function(data, status,xhr){	
										...
										if (resultArray[0]=='FAILED'){
											...
											$("#error_login").html(resultArray[1]);
													...
													//localStorage.doc_degree=resultArray[43]
													
													
													//<!--=====2020_01_02_jolly=====-->
													localStorage.productcomStr =resultArray[43]
													localStorage.doc_degree=resultArray[44]

													localStorage.geolocation=resultArray[45]
													localStorage.doc_offer=resultArray[46]


													//alert (localStorage.productcomStr)
													var productcomStrList = localStorage.productcomStr.split('<rd>');
													var productcomStrListShowLength=productcomStrList.length	
													//alert (resultArray[44])
													var productCombo1='';								
													$('#productl_combo').empty();
													$('#product2_combo').empty();
													$('#product3_combo').empty();
													var opt='<option value="">Select</option>'
													for (var k=0; k < productcomStrListShowLength; k++){
														var productValueArray = productcomStrList[k].split('<fd>');
														p1_ID=productValueArray[0];
														p1_Name=productValueArray[1];
														productID_1=p1_ID
														productName_1=p1_Name
														var product1_NameID=productID_1+'|'+productName_1;

													
														 opt = opt+'<option value="'+product1_NameID+'">'+product1_NameID+'</option>'
													...
													localStorage.visit_product_combo_1=opt;
...
//													}
													...
													for (var l=0; l < linkStrList.length; l++){
														...
														if(path_value!=''){
															...
															linkStr_combo+='<li style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><table class="buttonLink" width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr><td><a href="#" style="color:#fff" onclick="'+linkPath+'">'+pathName+'</a></td></tr></table></li>';
...
//												alert (localStorage.menu_list);
										...
										//------------- Visit Plan Market List / Client Profile Market List / Unschedule
													...
													for (var k=0; k < planMarketListShowLength; k++){
														...
														//alert (marketNameID);
														if(planMarketID!=''){
															unscheduleMarketComb+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a onClick="marketNextLV(\''+marketNameID+'\')"><font class="name" style="font-size:18; font-weight:bold">'+marketNameID+'</a></font></li>';
															
															visitPlanMarketComb+='<li  style=" border:1px solid #CBE4E4;color:#fff;height=15px" onClick="check_boxTourTrue(\''+marketID+'\')"> '+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox" onClick="getDocTour_keyup(\''+marketID+'\')" name="doc_tour'+marketID+'" value="checkbox" id="doc_tour'+marketID+'"><label for="doc_camp'+marketID+'"></br></label></td><td  style="text-align:left;">'+'</br><font id="'+ marketID +'" onClick="check_boxTourTrue(\''+marketID+'\')" class="name" >'+ marketNameID+'</font></td></tr>'+'</table>'+'</li>';
															
															profileMarketComb+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border:1px solid #CBE4E4;color:#fff;height:15px"><a onClick="marketNextCProfileLV(\''+marketNameID+'\')"><font class="name" style="font-size:12; font-weight:bold; color:#fff">'+marketNameID+'</font></a></li>';
															
															unscheduleMarketComb_tr+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a onClick="summary_report_doctor_tr(\''+marketNameID+'\')"><font class="name" style="font-size:18; font-weight:bold">'+marketNameID+'</a></font></li>';

															
															//	 2019-01-31 start														
															
														unscheduleFarmComb_tr+='<option style="font-size:12px; color:#fff; background-color:#ECECFF" value="'+marketNameID+'"><font style="font-size:12px; color:#fff; background-color:#ECECFF">'+marketNameID+'</font></option>'														
										...
										//===========Market===========================
												//------------- Visit Plan Market List / Client Profile Market List / Unschedule
													...
													if ((docMarketDate!=CDate) &&(tomorrowFlag==0) && (docMarketDate.length > 5)) {
																docMarketComb+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border:1px solid #CBE4E4;color:#fff;background-color:#EDFBFE"><font  style="font-size:24; font-weight:bold;color:#009">Tomorrow</font></li>';
																tomorrowFlag=1
															}
															if ((docMarketDate==CDate) &&(todayFlag==0)) {
																docMarketComb+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border:1px solid #CBE4E4;color:#fff;background-color:#EDFBFE"><font  style="font-size:24; font-weight:bold;color:#009">Today</font></li>';
																todayFlag=1
															}
															if (docMarketID!=''){
																docMarketComb+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a onClick="setScheduleDateDoc(\''+docMarketDate+'\');marketNextLV(\''+docmarketNameID+'\');"><font class="name" style="font-size:18; font-weight:bold">'+docMarketName+'</a></font></li>';
															//alert (docMarketComb)	
																}
														}
...
//													$('#tour_market_combo_id_lv').append(localStorage.visit_plan_marketlist_combo);
									...
									for (var c=0; c < planMarketListShowLengthCteam; c++){
											...
											if(planMarketIDCteam!='' && localStorage.cTeam==1){
												visitPlanMarketCombCteam+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a onClick="marketNextLV(\''+marketNameIDCteam+'\')"><font class="name" style="font-size:18; font-weight:bold">'+marketNameIDCteam+'</a></font></li>';
...
localStorage.report_button=' <input type="submit" id="loginButton" onClick="s_order_summary_report();"  style="width:100%;height: 40px;background-color: #6696a9;color:#FFF;font-size: 17px;border: 1px solid #6696a9;" value="    Sales Call and Order Count     "   /></br><input type="submit" id="loginButton" onClick="s_order_detail_report();"  style="width:100%;height: 40px;background-color: #6690a9e8;color:#FFF;font-size: 17px;border: 1px solid #5c9a97;" value="    Sales Call and Order Detail     "   />'

localStorage.report_button_tr='<input type="submit" id="loginButton" onClick="s_order_summary_report_tr_next();"  style="width:100%;height: 40px;background-color: #467484;color:#FFF;font-size: 17px;border: 1px solid #467484;" value="    Order Summary     "   />'
//localStorage.report_button='<a data-role="button" onClick="s_order_detail_report();" >Sales Call and Order Report</a>'
										...
										localStorage.doctor_report_button='<input type="submit" id="loginButton" onClick="summary_report_doctor();"  style="width:100%;height: 40px;background-color: #a79278;color:#FFF;font-size: 17px;border: 1px solid #a79278;" value="    DCR Count     "   /></br><input type="submit" id="loginButton" onClick="detail_report_doctor();" style="width:100%;height: 40px;background-color: #7bada1;color:#FFF;font-size: 17px;border: 1px solid #7bada1;" value="    DCR Detail     "   />'
	
										localStorage.doctor_report_button_tr='<input type="submit" id="loginButton" onClick=" summary_report_doctor_tr_next();"  style="width:100%;height: 40px;background-color: #688892;color:#FFF;font-size: 17px;border: 1px solid #688892;" value="    DCR Summary     "   />'
											}
	...
	// localStorage.prescription_report_button='<input type="submit" id="loginButton" onClick="summary_report_prescription();"  style="width:100%;height: 40px;background-color: #7496ab;color:#FFF;font-size: 17px;border: 1px solid #7496ab;" value="   Prescription Count     "   /></br><input type="submit" id="loginButton" onClick="detail_report_prescription();"  style="width:100%;height: 40px;background-color: #8193abde;color:#FFF;font-size: 17px;border: 1px solid #8193abde;" value="   Prescription Detail     "   />'
...
//		localStorage.prescription_report_button=''}
	...
	localStorage.report_others_tr	='<input type="submit" id="loginButton" onClick="pay_tr_next ();"  style="width:100%;height: 40px;background-color: #618f94;color:#FFF;font-size: 17px;border: 1px solid #618f94;" value="   Pay Slip      "   /></br><input type="submit" id="loginButton" onClick="expense_tr_next();"  style="width:100%;height: 40px;background-color: #649c97;color:#FFF;font-size: 17px;border: 1px solid #648e9c;" value="   Expense Slip     "   />'
...
//													
													...
													for (j=0; j < productLength; j++){
														...
														product_tbl_order=product_tbl_order+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="tr_item(\''+product_id2+'\')">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"  ><td width="60px" style="text-align:center; padding-left:5px;"><input class="orderProduct" maxlength="4" onChange="getOrderData_keyup(\''+product_id2+'\')" type="number" id="order_qty'+product_id2+'"  value="'+product_qty+'" placeholder="0" ><input type="hidden" id="order_id'+product_id2+'" value="'+product_id2+'" ><input type="hidden" id="order_price'+product_id2+'" value="'+product_price+'" ><input type="hidden" id="order_vat'+product_id2+'" value="'+vat+'" ><input type="hidden" id="order_name'+product_id2.toUpperCase()+'" value="'+product_name2.toUpperCase()+'" placeholder="qty" ><input type="hidden" id="order_promo'+product_id2.toUpperCase()+'" value="'+product_str+'" placeholder="qty" ></td><td>  </td><td  style="text-align:left; color:#fff" >'+'<font class="name" id="'+ product_id2 +'" onClick="tr_item(\''+product_id2+'\')" >'+ product_name2.toUpperCase()+'</font> | '+'<font class="itemCode">'+ product_id2.toUpperCase()+' | '+product_price+'</font><span id="stockShow'+product_id2.toUpperCase()+'" style="color:#600"></span></br> <span style="background-color:#FFFF53; color:#F00" id="promoShow'+product_id2.toUpperCase()+'" style="font-size:12px">'+product_str+'</span></td></tr>'+'</table>'+'</li>';
														//------------ Doctor Campaign Item list
														$("#error_login").html('Processing Product List....1');
														
														
														//alert ('1.................')
														product_tbl_doc_campaign=product_tbl_doc_campaign+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="check_boxTrue(\''+product_id2+'\')"> '+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox" onClick="getDocCampaignData_keyup(\''+product_id2+'\')" name="doc_camp'+product_id2+'" value="checkbox" id="doc_camp'+product_id2+'"><label for="doc_camp'+product_id2+'"></br></label><input type="hidden" id="doc_camp_id'+product_id2+'" value="'+product_id2+'" ><input type="hidden" id="doc_camp_price'+product_id2+'" value="'+product_price+'" ><input type="hidden" id="doc_camp_name'+product_id2.toUpperCase()+'" value="'+product_name2.toUpperCase()+'" placeholder="qty" ></td><td  style="text-align:left;color:#fff">'+'</br><font id="'+ product_id2 +'" onClick="tr_item_doc_campaign(\''+product_id2+'\')" class="name" >'+ product_name2.toUpperCase()+'</font></td></tr>'+'</table>'+'</li>';

														$("#error_login").html('Processing Product List....2');	
														//-------------Sample----------
														
														product_tbl_doc_sample=product_tbl_doc_sample+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="tr_sample(\''+product_id2+'\')">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="80px" style="text-align:center; padding-left:5px;"><input class="docSample" maxlength="4" onChange="getSampleData_keyup(\''+product_id2+'\')" type="number" id="sample_qty'+product_id2+'"  value="'+product_qty+'" placeholder="0" ><input type="hidden" id="sample_id'+product_id2+'" value="'+product_id2+'" ><input type="hidden" id="sample_price'+product_id2+'" value="'+product_price+'" ><input type="hidden" id="sample_name'+product_id2.toUpperCase()+'" value="'+product_name2.toUpperCase()+'" placeholder="qty" ></td><td  style="text-align:left;>  <font  class="name" >'+product_name2.toUpperCase()+'</font></td></tr>'+'</table>'+'</li>';
...
//	
										...
										if (localStorage.gift_string.length > 5 ){
											...
											for (j=0; j < giftLength; j++){
												...
												gift_tbl_doc=gift_tbl_doc+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="tr_gift(\''+gift_id+'\')">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="80px" style="text-align:center; padding-left:5px;color"#fff>'+'<input type="hidden" id="gift_id'+gift_id+'" value="'+gift_id+'" ><input class="docGift" maxlength="4" onChange="getGiftData_keyup(\''+gift_id+'\')" type="number" id="gift_qty'+gift_id+'"  value="" placeholder="0" ><input type="hidden" id="doc_gift_name'+gift_id.toUpperCase()+'" value="'+gift_name.toUpperCase()+'" placeholder="qty" ></td><td  style="text-align:left;">'+'  <font id="'+ gift_name +'" onClick="tr_item_doc_campaign(\''+gift_id+'\')" class="name" style="color:#fff">'+ gift_name.toUpperCase()+'</font></td></tr>'+'</table>'+'</li>';
...
//													========================PPM Start========
													if (localStorage.ppm_string.length > 5 ){
														...
														for (j=0; j < ppmLength; j++){
															...
															ppm_tbl_doc=ppm_tbl_doc+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="tr_ppm(\''+ppm_id+'\')">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="80px" style="text-align:center; padding-left:5px;">'+'<input type="hidden" id="ppm_id'+ppm_id+'" value="'+ppm_id+'" ><input class="docPpm" maxlength="4" style="color:#000" onChange="getppmData_keyup(\''+ppm_id+'\')" type="number" id="ppm_qty'+ppm_id+'" value="" placeholder="0" ><input type="hidden" id="doc_ppm_name'+ppm_id.toUpperCase()+'" value="'+ppm_name.toUpperCase()+'" placeholder="qty" ></td><td  style="text-align:left;">'+'  <font style="color:#fff" id="'+ ppm_name +'" onClick="tr_item_doc_campaign(\''+ppm_id+'\')" class="name">'+ ppm_name.toUpperCase()+'</font></td></tr>'+'</table>'+'</li>';
...
//	$('#bonus_combo').empty();
//	$("#bonus_combo").append('<option style="font-size:12px; color:#fff; background-color:#ECECFF" value="0"><font style="font-size:12px; color:#fff; background-color:#ECECFF">N/A</font></option>')
//	for (j=0; j < bonusComboList.length; j++){
//		var single_promo=bonusComboList[j].split('<fd>');
//		$("#bonus_combo").append('<option style="font-size:12px; color:#fff; background-color:#ECECFF" value="'+single_promo[2]+' ('+single_promo[0]+')'+'"><font style="font-size:12px; color:#fff; background-color:#ECECFF">'+single_promo[2]+'('+single_promo[0]+')'+'</font></option>');
//	}
//	
//}
...
function chemist_profile() {
	$("#ret_cat").show();
	$("#d_visit").html("Chemist");
	//$("#v_path").html('<font style="font-weight:bold; font-size:13px; color:#fff">Visit > Market > Chemist</font>');
	localStorage.doctor_flag=0;
...
function doctor_profile() {
	...
	$("#ret_cat").hide();
	$("#d_visit").html("Doctors");
	//$("#v_path").html('<font style="font-weight:bold; font-size:13px; color:#fff">Visit > Market > Doctor</font>');
...
function tour_amnd(){
	...
	// alert (localStorage.path_value_report)

	var linkPath="window.open('"+localStorage.path_value_report+"teamShow_web?"+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"&monthPass=Next', '_system');"
	
	var linkPath1="window.open('"+localStorage.path_main+"track_mobile/tracking_index?"+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"&monthPass=Next', '_system');"
	...
	var linkPath_team_web="window.open('"+localStorage.path_value_report+"teamShow_web?"+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"&monthPass=Next', '_system');"
    
    var tour_combo='<img style="padding-top:0px; padding-bottom:0px;" hight="60px" width="60px" src="tour.png" onclick="'+linkPath+'">';
	
	var tour_combo1='<img style="padding-top:0px; padding-bottom:0px;" hight="60px" width="60px" src="activity_log.png" onclick="'+linkPath1+'">';
	

	var tour_combo2='<img style="padding-top:0px; padding-bottom:0px;" hight="60px" width="60px" src="amnd.png" onclick="'+linkPath2+'">';
    var team_web='<img style="padding-top:0px; padding-bottom:0px;" hight="60px" width="60px" src="team.png" onclick="'+linkPath_team_web+'">';
	// alert ('Test')		
	 $('#tourDiv').empty();
	 $('#tourDiv').append(tour_combo);	
	
	 $('#activity_logVDiv').empty();
	 $('#activity_logVDiv').append(tour_combo1);
	
	$('#amndVDiv').empty();
	$('#amndVDiv').append(tour_combo2);

	$('#teamShow').empty();
	$('#teamShow').append(team_web);	
...
function tour(){
	...
	var linkPath2="window.open('"+localStorage.path_value_tour+"amndShow_web?"+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"&monthPass=Next', '_system');"
			...
			var tour_combo='<img style="padding-top:0px; padding-bottom:0px;" hight="100px" width="100px" src="this_month.png" onclick="'+linkPath+'">';
			var tour_combo1='<img style="padding-top:0px; padding-bottom:0px;" hight="100px" width="100px" src="next_month.png" onclick="'+linkPath1+'">';
...
function page_pending(){
	...
	$.ajax(localStorage.base_url+'tourPending?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode,{
		...
		var linkPath="window.open('"+localStorage.path_value_report+"teamShow_web?"+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"&monthPass=This', '_system');"
			...
			var tour_combo='<img style="padding-top:0px; padding-bottom:0px;" hight="100px" width="100px" src="team.png"  onclick="'+linkPath+'">';
			$('#teamSS').empty();
...
function reports() {
	var str_report_rep='<table width="100%" border="0">'+
					 '<tr><td style="color:#fff">ID: </td><td ><input id="se_mpo_doc" name="se_mpo_doc" type="text" readonly="true" placeholder="Rep">'+
					  '<input id="se_item_doc" name="se_item_doc" type="hidden" placeholder="Item"></td></tr>'+
					   '<tr><td style="color:#fff">BaseCode: </td><td><input id="se_market_doc" name="se_market_doc" type="text" placeholder="Market"  ></td></tr></table>'
	var str_report_sup='<table width="100%" border="0">'+
					   '<tr><td style="color:#fff">ID: </td><td><input id="se_mpo_doc" name="se_mpo_doc" type="text" placeholder="Rep">'+
					   '<input id="se_item_doc" name="se_item_doc" type="hidden" placeholder="Item"></td></tr>	'+
					   '<tr><td style="color:#fff">BaseCode: </td><td><input id="se_market_doc" name="se_market_doc" type="text" placeholder="Market/level"  ></td></tr></table>'	
...
function repCancelReqShow(i){
	...
	for (var m=0; m < marketList.length; m++){
		...
		if (marketId!=''){
		amndTable=amndTable+'<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="'+checkId+'" value="checkbox" id="'+checkId+'"><label for="'+checkId+'"></br></label></td><td  style="text-align:left;"></br>'+marketName+'</br></td></tr></table>'
		}
		else{
			amndTable=amndTable+'<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"></br></td></tr></table>'
		}
		
	}
	//amndLeave='<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="amndOthers" value="checkbox" id="amndOthers" ><label for="amndOthers"></br></label></td><td  style="text-align:left;"></br>Office Work / Training</br></br></td></tr></table>'
	//amndLeave=amndLeave+'<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="amndleav" value="checkbox" id="amndleav" ><label for="amndleav"></br></label></td><td  style="text-align:left;"></br>Leave</br></br></td></tr></table>'
...
function repCancelReqShowNext(i){
	...
	for (var m=0; m < marketList.length; m++){
		...
		if (marketName==''){marketIdShow=marketId}
		if (marketId!=''){
		amndTable=amndTable+'<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="'+checkId+'" value="checkbox" id="'+checkId+'"><label for="'+checkId+'"></br></label></td><td  style="text-align:left;"></br>'+marketName+marketIdShow+'</br></br></td></tr></table>'
		}
		else{
			amndTable=amndTable+'<table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"></br></td></tr></table>'
		}
	...
	if (MvalueFlag==1){
			...
			for (var i=0; i < daysNext; i++){
				...
				for (var m=0; m < marketList.length; m++){
					...
					if (marketName==''){ marketIdShow=''+marketId}
					if (marketId!=''){
					nextMonthTable=nextMonthTable+'<div id="next_'+i+'"><table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="'+checkId+'" value="checkbox" id="'+checkId+'"><label for="'+checkId+'"></br></label></td><td align="left"></br>'+marketName+'</br></td></tr></table>'
				//	nextMonthTable=nextMonthTable+'<div id="next_'+i+'"><table width="100%" border="0"  cellpadding="0" cellspacing="0" style="border-radius:5px;"><tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input class="docCampaign" type="checkbox"  name="'+checkId+'" value="checkbox" id="'+checkId+'"><label for="'+checkId+'">'+marketName+'</label></td></tr></table>'
					...
					if 	(m_client_string=='Retailer not available'){
						...
						for (var i=0; i < mClientListShowLength; i++){
							...
							if(mClientID!=''){
									...
									unscheduled_m_client_list+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff;padding:8px 5px"><table><tr><td><img onClick="page_chemist_profile(\''+mClientName+'|'+mClientID+'\')" style="height:20px; width:20px" src="editProfile.png">    </td><td><a  onClick="marketRetailerNextLV(\''+mClientName+'|'+mClientID+'\')"><font class="name" style="font-size:18; font-weight:600; color:fff">'+mClientName+'| </font>'+mClientID+'</font></a></td></table></li>';
							}
						 }
	...
	//$.post(localStorage.base_url+'getMarketClientList?',{cid: localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,market_id:market_Id,catType:catType},
						...
						//					-----------------------------------
								...
								for (var i=0; i < mClientListShowLength; i++){
										...
										var mClientName=mClientValueArray[1];
										var mClientCat=mClientValueArray[2];
										unscheduled_m_client_list+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><table><tr><td><img onClick="page_chemist_profile(\''+mClientName+'|'+mClientID+'\')" style="height:20px; width:20px" src="editProfile.png">    </td><td><a  onClick="marketRetailerNextLV(\''+mClientName+'|'+mClientID+'\')"><font class="name" style="font-size:18; font-weight:600; color:#fff">'+mClientName+'| </font>'+mClientID+'</font></a></td></tr></table></li>'	
										//unscheduled_m_client_list+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a  onClick="marketRetailerNextLV(\''+mClientName+'|'+mClientID+'\')"><font>     '+mClientName+'|'+mClientID+','+mClientCat+'</font></a></li>';
									}
...
//			$("#wait_image_unschedule_market_ret").show();		
			...
			// alert (lat_lon_show=localStorage.latitude+","+localStorage.longitude)
			// var lat_lon_show=localStorage.latitude+","+localStorage.longitude
			// $("#latlong_show").html(lat_lon_show);
			if (localStorage.visit_location_flag!='YES'){
				//alert (localStorage.visit_location);
	...
	if (localStorage.productOrderStr.length >0){
		...
		for (var j=0; j < orderProductLength; j++){
			...
			if(orderProductIdQtyList.length==2){
				...
				//product_tbl_cart_str=product'_tbl_cart_str+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff">'+'</li>'
				//alert (product_name)
				product_tbl_cart_str=product_tbl_cart_str+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;" onClick="tr_item_cart(\''+orderProductId+'\')">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr style="border-radius:0px;border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;"><input  type="number" id="cart_qty'+orderProductId+'"  onBlur="getCartData_keyup(\''+orderProductId+'\')" value="'+orderProductQty+'" placeholder="0"> </td><td>&nbsp;</td><td  style="text-align:left;color:#fff">'+ product_name.toUpperCase()+' | '+orderProductId+' | '+product_price+' '+'<span style="color:#600">'+stock_str_cart+'</span>'+'</br> <span style="background-color:#FFFF53; color:#F00;font-size:12px">'+promo_str_cart+'</span>'+'<input  type="hidden" id="cart_vat'+orderProductId+'"  value="'+prom_flag+'" >'+'</td></tr>'+'</table>'+'</li>'
						...
						for (var i=0; i < mClientListShowLength; i++){
							...
							if (mClientID!=''){
								if ((localStorage.doctor_flag==1) & (localStorage.doctor_plan_flag==0) & (localStorage.doctor_pr==0)){
									unscheduled_m_client_list+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><table><tr><td><img onClick="page_doctor_profile(\''+mClientName+'|'+mClientID+'\')" style="height:20px; width:20px" src="editProfile.png">    </td><td><a onClick="marketRetailerNextLV(\''+mClientName+'|'+mClientID+'\')"><font class="name" style="font-size:18; font-weight:600; color:#fff">'+mClientName+'|'+mClientID+'</font></a></td></tr></table></li>';
									// alert ('<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><a onClick="marketRetailerNextLV(\''+mClientName+'|'+mClientID+'\')"><font class="name" style="font-size:18; font-weight:600; color:#fff">'+mClientName+'|'+mClientID+'</font></a></li>')
...
function marketRetailerNext_doc() {
		...
		if (dt.getHours() > 12 ) {today=today+ ' PM'} else {today=today+ ' AM'};
		var docVisitTime=today
		// var docVisitTime = hour + ":" + dt.getMinutes() + ":" + dt.getSeconds();
		
		localStorage.docVisitTime=docVisitTime
		alert (docVisitTime)
...
function getDocSampleData(){	
	...
	for (var j=0; j < sampleProductLength; j++){
			if (sampleProductList[j] != ''){
				...
				sample_show_1=sample_show_1+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr><td >'+pname+'('+sampleProductsingleList[0]+')'+'</td><td width="80px">'+'<input  type="number" style="color:#000" id="s_cart_qty'+ sampleProductsingleList[0] +'"  onBlur="sampleCartData_keyup(\''+sampleProductsingleList[0] +'\');" value="'+sampleProductsingleList[1]+'" placeholder="0">'+'</td></tr>'+'</table>'+'</li>'
			}
		}
...
function getDocGiftData(){	
	...
	for (var j=0; j < gift_showListLength; j++){
		...
		gift_show_1=gift_show_1+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr><td  >'+pname+'('+giftProductsingleList[0]+')'+'</td><td width="80px">'+'<input  type="number" style="color:#000" id="g_cart_qty'+ giftProductsingleList[0] +'"  onBlur="giftCartData_keyup(\''+giftProductsingleList[0] +'\');" value="'+giftProductsingleList[1]+'" placeholder="0">'+'</td></tr>'+'</table>'+'</li>'
	}
	if (gift_show_1!=''){
...
function getDocppmData(){	
	...
	for (var j=0; j < ppm_showListLength; j++){
		...
		ppm_show_1=ppm_show_1+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff">'+'<table width="100%" border="0" id="order_tbl" cellpadding="0" cellspacing="0" style="border-radius:5px;">'+'<tr><td  >'+pname+'('+ppmProductsingleList[0]+')'+'</td><td width="80px" >'+'<input style="color:#000"  type="number" id="g_cart_qty'+ ppmProductsingleList[0] +'"  onBlur="ppmCartData_keyup(\''+ppmProductsingleList[0] +'\');" value="'+ppmProductsingleList[1]+'" placeholder="0">'+'</td></tr>'+'</table>'+'</li>'
	}
	if (ppm_show_1!=''){
...
function visitSubmit_doc(){	
	...
	var productl_combo= $("#productl_combo").val()
	var product2_combo= $("#product2_combo").val()
	var product3_combo= $("#product3_combo").val()
	
	var docVisitTime=localStorage.docVisitTime
...
//	}
//	else{
			if (v_with=='' || v_with==undefined || v_with=='undefined'){// || (productl_combo=='' && product2_combo='' && product3_combo=='')){
				$("#errorChkVSubmit_doc").html('Visited with or Reminder Product not selected');		
			}else{
		...
		//		}else{
										...
										//alert (localStorage.base_url+'doctor_visit_submit_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&client_id='+visitClientId+'&visit_type='+visit_type+'&schedule_date='+scheduled_date+'&msg=' +encodeURI(msg)+'&lat='+lat+'&long='+longitude+'&v_with='+v_with+'&route='+market_Id+'&doc_others='+doc_others+'&location_detail='+localStorage.location_detail+'&imageName='+imageName+'&v_shift='+v_shift)	
										$.ajax(localStorage.base_url+'doctor_visit_submit_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&client_id='+visitClientId+'&visit_type='+visit_type+'&schedule_date='+scheduled_date+'&msg=' +encodeURI(msg)+'&lat='+lat+'&long='+longitude+'&v_with='+v_with+'&route='+market_Id+'&doc_others='+doc_others+'&location_detail='+localStorage.location_detail+'&imageName='+imageName+'&v_shift='+v_shift+'&productl_combo='+productl_combo+'&product2_combo='+product2_combo+'&product3_combo='+product3_combo+'&docVisitTime='+docVisitTime,{
										// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
										type: 'POST',
										timeout: 30000,
										error: function(xhr) {
...
function after_save_data(){
	...
	//--
	$("#visit_save").html('</br><font style="color:#87da9d;">Saved in your mobile. Please submit from saved order when you have good network. </font>');
...
function getvisitSave_data(){
	...
	for (var i=0; i < saved_dataArray.length -1 ; i++){
	   ...
	   saved_data_list=saved_data_list+'<li  style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><table width="100%" border="0"><tr><td width="2%"></td><td width="80%" onClick="set_save_data('+i+')"  >'+'<font style="font-size:15px ;color:#fff"><b>'+visit_client_show+'</b></font><font style="color:#ddd;font-size:13px">( '+visit_market_show+' )'+'</font></br><font style="color:#b1d4a9"> '+visit_time_show+'</font></td><td width="18%" align="right">' +'<input type="submit" style="width:100%;height: 30px;width: 35px;background-color: #bf6979;color:#FFF;font-size:15px;border: 1px solid #ddd;" onClick="cancelSave('+i+')" value="X"  >' +'</font>' +'</td> </tr></table></li>'
	  // alert (client_id);
...
//		//alert ('nnnn')
//		
//		$("#bonus_combo").append('<option style="font-size:12px; color:#fff; background-color:#ECECFF" value="'+bonus_combo+'"><font style="font-size:12px; color:#fff; background-color:#ECECFF">'+bonus_combo+'</font></option>');
//	}
//	$("#bonus_combo").append('<option style="font-size:12px; color:#fff; background-color:#ECECFF" value="0"><font style="font-size:12px; color:#fff; background-color:#ECECFF">N/A</font></option>')
//	var promo_str=localStorage.promo_str;
//	var bonusComboList=promo_str.split('<rd>');
	...
	// ajax-------
			...
			$.ajax(localStorage.report_url+'report_summary_doctor?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.rep_id_report_doc+'&se_item_report='+localStorage.se_item_report_doc+'&se_market_report='+localStorage.se_market_report_doc+'&date_from='+localStorage.date_from_doc+'&date_to='+localStorage.date_to_doc+'&user_type='+localStorage.user_type,{
								...
								if (resultArray[0]=='FAILED'){
											...
											$("#visit_count_doctor").html("<font style='font-size:15px; color:#fff'>"+"visit Count:"+visit_count+"</font>");
											$("#visit_withAtt_doctor").html("<font style='font-size:15px; color:#fff'>"+visit_areawise+"</font>");
											$("#visit_withoutAtt_doctor").html("<font style='font-size:15px; color:#fff'>"+visit_repwise+"</font>");
	...
	// ajax-------
			...
			$.ajax(localStorage.report_url+'report_summary_prescription?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.rep_id_report_doc+'&se_item_report='+localStorage.se_item_report_doc+'&se_market_report='+localStorage.se_market_report_doc+'&date_from='+localStorage.date_from_doc+'&date_to='+localStorage.date_to_doc+'&user_type='+localStorage.user_type,{
							...
							//	$("#err_retailer_date_next").text("");
								...
								$("#visit_count_prescription").html("<font style='font-size:15px; color:#fff'>"+"Prescription Count:"+visit_count+"</font>");
								//$("#visit_withAtt_prescription").html("<font style='font-size:15px; color:#fff'>"+visit_areawise+"</font>");
								//$("#visit_withoutAtt_prescription").html("<font style='font-size:15px; color:#fff'>"+visit_repwise+"</font>");
...
// ======================= 20210515 sales order Report================
function salesOrdr_smry_rept(){
	$("#wait_image_order").show();
	
	$("#myerror_s_reporttxt").val(localStorage.report_url+'salsCall_order_smry?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.rep_id_report_doc+'&se_item_report='+localStorage.se_item_report_doc+'&se_market_report='+localStorage.se_market_report_doc+'&date_from='+localStorage.date_from_doc+'&date_to='+localStorage.date_to_doc+'&user_type='+localStorage.user_type);
		alert (localStorage.report_url+'salsCall_order_smry?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.user_id+'&user_type='+localStorage.user_type)
		$.ajax(localStorage.report_url+'salsCall_order_smry?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.user_id+'&user_type='+localStorage.user_type,{

								type: 'POST',
								timeout: 30000,
								error: function(xhr) {
									$("#wait_image_order").hide();
									$("#slsOrdSumry_error").html('Network Timeout. Please check your Internet connection..');
													},
								success:function(data, status,xhr){	
								$("#wait_image_order").hide();
									 if (status!='success'){
								...
								if (resultArray[0]=='FAILED'){
											$("#slsOrdSumry_error").text(resultArray[0]);	
								...
								$("#sales_report_shw").html(result_string);
							
								//-----------------
								$("#slsOrdSumry_error").html("");
	...
	$.afui.loadContent("#sales_order_summary_report",true,true,'right');
...
function salesReport_dcr() {
	$("#wait_image_orderdcr").show();		
	// set_report_parameter_doctor();
	$.ajax(localStorage.report_url+'salesRprt_doctor?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&user_type='+localStorage.user_type,{
		type: 'POST',
			timeout: 30000,
			error: function(xhr) {
				$("#wait_image_orderdcr").hide();
				$("#slsRptdcr_error").html('Network Timeout. Please check your Internet connection..');
			},
			success:function(data, status,xhr){	
			$("#wait_image_orderdcr").hide();
				 if (status!='success'){
			...
			if (resultArray[0]=='FAILED'){
						$("#slsRptdcr_error").text(resultArray[0]);	
						
					}
			else if (resultArray[0]=='SUCCESS'){

			var result_string=resultArray[1];
			$("#sales_reportdcr_shw").html(result_string);
		
			//-----------------
			$("#slsRptdcr_error").html("");
			$("#report_header").text("Sales Call and Order Count");
			$("#rep_detail").html("");
			//-----
	...
	$.afui.loadContent("#sales_report_dcr",true,true,'right');
...
function s_order_summary_report() {		
	...
	// ajax-------
		$.ajax(localStorage.report_url+'s_call_order_summary?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.rep_id_report_doc+'&se_item_report='+localStorage.se_item_report_doc+'&se_market_report='+localStorage.se_market_report_doc+'&date_from='+localStorage.date_from_doc+'&date_to='+localStorage.date_to_doc+'&user_type='+localStorage.user_type,{
				...
				$("#report_header").text("Sales Call and Order Count");
				$("#sales_call").html("<font style='font-size:15px; color:#fff'>"+"Sales Call:"+"</div>"+sales_call);
				$("#order_count").html("<font style='font-size:15px; color:#fff'>"+"Order Count:"+"</div>"+order_count);
				$("#order_value").html("<font style='font-size:15px; color:#fff'>"+"Order Value:"+"</div>"+order_value);
	...
	// ajax-------
		$.ajax(localStorage.report_url+'s_call_order_detail?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&rep_id_report='+localStorage.rep_id_report_doc+'&se_item_report='+localStorage.se_item_report_doc+'&se_market_report='+localStorage.se_market_report_doc+'&date_from='+localStorage.date_from_doc+'&date_to='+localStorage.date_to_doc+'&user_type='+localStorage.user_type,{
								...
								if (resultArray[0]=='FAILED'){
									...
									$("#sales_call").html("<font style='font-size:15px; color:#fff'>"+"Sales Call:"+"</font>"+sales_call);
									$("#order_count").html("<font style='font-size:15px; color:#fff'>"+"Order Count:"+"</font>"+order_count);
									$("#order_value").html("<font style='font-size:15px; color:#fff'>"+"Order Value:"+"</font>"+order_value);
	...
	// alert (localStorage.report_url+'doc_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&route='+market_Id+'&docId='+visitDocId)
		$.ajax(localStorage.report_url+'doc_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&route='+market_Id+'&docId='+visitDocId,{
								...
								if (resultArray[0]=='FAILED'){
									...
									var third_party_id=result_string.split('<fdfd>')[13]
									var service_id=result_string.split('<fdfd>')[14]
									var service_kol_dsc=result_string.split('<fdfd>')[15]
									var at_ins=result_string.split('<fdfd>')[16]
									var micro_U=result_string.split('<fdfd>')[17]
									// alert (service_kol_dsc)
									...
									$("#dS_K_D").val(service_kol_dsc)
									$("#dAttachedInstitute").val(at_ins)                           			
                           			$("#dDistrict").val(micro_U)
									...
									for (var j=0; j < catList.length-1; j++){
										        // '<option value="'+desigList[d]+'" selected>'+desigList[d]+'</option>'
										var opt='<option value="'+catList[j]+'">'+catList[j]+'</option>'
									...
									for (var s=0; s < spacialityList.length-1; s++){
										var opti='<option value="'+spacialityList[s]+'">'+spacialityList[s]+'</option>'
										 $('#dSpaciality').append(opti);
										 // alert ('dSpaciality')
										 
									}
									
									 // var doc_degree=localStorage.doc_degree	
									// alert (doc_degree)										
									doc_degreeList=dDegree.split(',')
									$('#dDegree').empty();
									
									for (var d=0; d < doc_degreeList.length-1; d++){
										var oppt='<option value="'+doc_degreeList[d]+'">'+doc_degreeList[d]+'</option>'
...
function docProfileSubmit() {
	...
	if (dCAddress=='' ||  dCategory=='' || dS_K_D==''){
		$("#myerror_doctor_prof").html('Please Complete Mandatory Fields.' )
		$("#wait_image_docProf").hide();
								...
								success:function(data, status,xhr){	
									 ...
									 if (status!='success'){
										...
										if (Cash_Credit==''){
											...
											$('#dCCash_Credit').append('<option selected value="Both">Both</option>')
										}
										else{
...
function page_addDoc() {
	...
	// alert (localStorage.doc_degree)
	var doc_degree=localStorage.doc_degree											
	doc_degreeList=doc_degree.split(',')
	$('#addDegree').empty();
	
	for (var j=0; j < doc_degreeList.length-1; j++){
		var opt='<option value="'+doc_degreeList[j]+'">'+doc_degreeList[j]+'</option>'
		
		 $('#addDegree').append(opt);
	}
...
function docAddSubmit() {
	...
	// #===========31_05_2021=========== 
	if (dCAddress=='' ||  dCategory=='' || dS_K_D=='' || dName==''){
...
// ===================================
// ====== DAIRY IMAGE 20211220
function getdiaryImage() {
	navigator.camera.getPicture( onSuccess_diaryImage, onFail_diaryImage, {
		quality: 90,
		targetWidth: 400,
       // destinationType: Camera.DestinationType.FILE_URI,
		destinationType: Camera.DestinationType.FILE_URI,correctOrientation: true ,
...
function onSuccess_diaryImage(imageURI) {
    var image = document.getElementById('diaryImage');
    image.src = imageURI;
	imagePath = imageURI;
	$("#diaryPhoto").val(imagePath);	
}
function onFail_diaryImage(message) {
	imagePath="";
    alert('Failed because: ' + message);
}

// ====== DAIRY IMAGE 20211220 END
// ===================================
...
function chemist_submit() {	
	...
	if  ((ChemistName=='') || (Address_Line_1=='') || (Contact_Name=='' ) ||  (Contact_phone=='') || (Category=='') || (Cash_Credit=='')){
		error_flag=1
		}
	//alert (error_flag)	
	if (error_flag==1){
...
// ====== DAIRY IMAGE 20211220
function diary_image() {	

	$("#wait_diaryImage").hide();
	
	$.afui.loadContent("#diary_image",true,true,'right');
}

function diaryImageSubmit(){	

	$("#errorDiaryImage").hide();
	$("#wait_diaryImage").hide();
  
	var diary_img_date=$("#diary_img_date").val(); 
	var diaryPhoto=$("#diaryPhoto").val(); 
	var lat=$("#lat").val();
	var longitude=$("#longitude").val();
	var now = $.now(); 
	...
	// alert (localStorage.base_url+'diaryImage_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&user_type='+localStorage.user_type+'&synccode='+localStorage.synccode+'&diaryPhoto='+imageName+'&diary_img_date='+diary_img_date)
	$.ajax(localStorage.base_url+'diaryImage_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&user_type='+localStorage.user_type+'&synccode='+localStorage.synccode+'&diaryPhoto='+imageName+'&diary_img_date='+diary_img_date,{
		// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
		type: 'POST',
		timeout: 30000,
		error: function(xhr) { 
			$("#wait_diaryImage").hide(); 
			$("#errorDiaryImage").html('Network Timeout. Please check your Internet connection..');
		},
		success:function(data, status,xhr){	
			if (status!='success'){
				$("#wait_diaryImage").show(); 
				$("#errorDiaryImage").html('Network Timeout. Please check your Internet connection...');
			}else{		 
			 	var resultArray = data.replace('</START>','').replace('</END>','').split('<SYNCDATA>');	
				if (resultArray[0]=='FAILED'){						
					$("#errorDiaryImage").html(resultArray[1]);
					$("#wait_diaryImage").show(); 	
				}else if (resultArray[0]=='SUCCESS'){
					uploadPhoto(diaryPhoto, imageName);		
					...
					$("#errorDiaryImage").html('');
					 
					$("#diaryPhoto").val('');
					document.getElementById('diaryImage').src = '';
					
					$("#wait_diaryImage").hide(); 
					$("#diary_image_success").html('Submitted Successfully');
					
					$.afui.loadContent("#page_diary_image_success",true,true,'right');			
				}else{						
					$("#errorDiaryImage").html('Network Timeout. Please check your Internet connection.');
					$("#wait_diaryImage").show(); 							
				}
...
function setOpProduct(){
	...
	//alert (localStorage.op_A)
		if (localStorage.op_A.length != '') {
			...
			for (j=0; j < opLength_A; j++){
				...
				op_tbl_A=op_tbl_A+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_A+'\')"  class="name"><font id="opName'+ op_id_A +'" class="name" >'+ op_name_A+'</font><input type="hidden" id="doc_op_id'+op_id_A+'" value="'+op_id_A+'" > '+'</li>';		
	...
	if (localStorage.op_B.length != '') {
			...
			for (j=0; j < opLength_B; j++){
				...
				op_tbl_B=op_tbl_B+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_B+'\')"  class="name"><font id="opName'+ op_id_B +'" class="name" >'+ op_name_B+'</font><input type="hidden" id="doc_op_id'+op_id_B+'" value="'+op_id_B+'" > '+'</li>';		
	...
	if (localStorage.op_C.length != '') {
			...
			for (j=0; j < opLength_C; j++){
				...
				op_tbl_C=op_tbl_C+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_C+'\')"  class="name"><font id="opName'+ op_id_C +'" class="name" >'+ op_name_C+'</font><input type="hidden" id="doc_op_id'+op_id_C+'" value="'+op_id_C+'" > '+'</li>';		
	...
	if (localStorage.op_D.length != '') {
			...
			for (j=0; j < opLength_D; j++){
				...
				op_tbl_D=op_tbl_D+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_D+'\')"  class="name"><font id="opName'+ op_id_D +'" class="name" >'+ op_name_D+'</font><input type="hidden" id="doc_op_id'+op_id_D+'" value="'+op_id_D+'" > '+'</li>';		
	...
	if (localStorage.op_E.length != '') {
			...
			for (j=0; j < opLength_E; j++){
				...
				op_tbl_E=op_tbl_E+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_E+'\')"  class="name"><font id="opName'+ op_id_E +'" class="name" >'+ op_name_E+'</font><input type="hidden" id="doc_op_id'+op_id_E+'" value="'+op_id_E+'" > '+'</li>';	
		...
		if (localStorage.op_F.length != '') {
			...
			for (j=0; j < opLength_F; j++){
				...
				op_tbl_F=op_tbl_F+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_F+'\')"  class="name"><font id="opName'+ op_id_F +'" class="name" >'+ op_name_F+'</font><input type="hidden" id="doc_op_id'+op_id_F+'" value="'+op_id_F+'" > '+'</li>';		
	...
	if (localStorage.op_G.length != '') {
			...
			for (j=0; j < opLength_G; j++){
				...
				op_tbl_G=op_tbl_G+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_G+'\')"  class="name"><font id="opName'+ op_id_G +'" class="name" >'+ op_name_G+'</font><input type="hidden" id="doc_op_id'+op_id_G+'" value="'+op_id_G+'" > '+'</li>';		
	...
	if (localStorage.op_H.length != '') {
			...
			for (j=0; j < opLength_H; j++){
				...
				op_tbl_H=op_tbl_H+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_H+'\')"  class="name"><font id="opName'+ op_id_H +'" class="name" >'+ op_name_H+'</font><input type="hidden" id="doc_op_id'+op_id_H+'" value="'+op_id_H+'" > '+'</li>';		
	...
	if (localStorage.op_I.length != '') {
			...
			for (j=0; j < opLength_I; j++){
				...
				op_tbl_I=op_tbl_I+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_I+'\')"  class="name"><font id="opName'+ op_id_I +'" class="name" >'+ op_name_I+'</font><input type="hidden" id="doc_op_id'+op_id_I+'" value="'+op_id_I+'" > '+'</li>';	
	...
	if (localStorage.op_J.length != '') {
			...
			for (j=0; j < opLength_j; j++){
				...
				op_tbl_J=op_tbl_J+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_J+'\')"  class="name"><font id="opName'+ op_id_J +'" class="name" >'+ op_name_J+'</font><input type="hidden" id="doc_op_id'+op_id_J+'" value="'+op_id_J+'" > '+'</li>';	
	...
	if (localStorage.op_K.length != '') {
			...
			for (j=0; j < opLength_K; j++){
				...
				op_tbl_K=op_tbl_K+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_K+'\')"  class="name"><font id="opName'+ op_id_K +'" class="name" >'+ op_name_K+'</font><input type="hidden" id="doc_op_id'+op_id_K+'" value="'+op_id_K+'" > '+'</li>';	
	...
	if (localStorage.op_L.length != '') {
			...
			for (j=0; j < opLength_L; j++){
				...
				op_tbl_L=op_tbl_L+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_L+'\')"  class="name"><font id="opName'+ op_id_L +'" class="name" >'+ op_name_L+'</font><input type="hidden" id="doc_op_id'+op_id_L+'" value="'+op_id_L+'" > '+'</li>';	
	...
	if (localStorage.op_M.length != '') {
			...
			for (j=0; j < opLength_M; j++){
				...
				op_tbl_M=op_tbl_M+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_M+'\')"  class="name"><font id="opName'+ op_id_M +'" class="name" >'+ op_name_M+'</font><input type="hidden" id="doc_op_id'+op_id_M+'" value="'+op_id_M+'" > '+'</li>';		
	...
	if (localStorage.op_N.length != '') {
			...
			for (j=0; j < opLength_N; j++){
				...
				op_tbl_N=op_tbl_N+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_N+'\')"  class="name"><font id="opName'+ op_id_N +'" class="name" >'+ op_name_N+'</font><input type="hidden" id="doc_op_id'+op_id_N+'" value="'+op_id_N+'" > '+'</li>';	
	...
	if (localStorage.op_O.length != '') {
			...
			for (j=0; j < opLength_O; j++){
				...
				op_tbl_O=op_tbl_O+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_O+'\')"  class="name"><font id="opName'+ op_id_O +'" class="name" >'+ op_name_O+'</font><input type="hidden" id="doc_op_id'+op_id_O+'" value="'+op_id_O+'" > '+'</li>';		
	...
	if (localStorage.op_P.length != '') {
			...
			for (j=0; j < opLength_P; j++){
				...
				op_tbl_P=op_tbl_P+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_P+'\')"  class="name"><font id="opName'+ op_id_P +'" class="name" >'+ op_name_P+'</font><input type="hidden" id="doc_op_id'+op_id_P+'" value="'+op_id_P+'" > '+'</li>';	
	...
	if (localStorage.op_Q.length != '') {
			...
			for (j=0; j < opLength_Q; j++){
				...
				op_tbl_Q=op_tbl_Q+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_Q+'\')"  class="name"><font id="opName'+ op_id_Q +'" class="name" >'+ op_name_Q+'</font><input type="hidden" id="doc_op_id'+op_id_Q+'" value="'+op_id_Q+'" > '+'</li>';	
	...
	if (localStorage.op_R.length != '') {
			...
			for (j=0; j < opLength_R; j++){
				...
				op_tbl_R=op_tbl_R+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_R+'\')"  class="name"><font id="opName'+ op_id_R +'" class="name" >'+ op_name_R+'</font><input type="hidden" id="doc_op_id'+op_id_R+'" value="'+op_id_R+'" > '+'</li>';		
	...
	if (localStorage.op_S.length != '') {
			...
			for (j=0; j < opLength_S; j++){
				...
				op_tbl_S=op_tbl_S+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_S+'\')"  class="name"><font id="opName'+ op_id_S +'" class="name" >'+ op_name_S+'</font><input type="hidden" id="doc_op_id'+op_id_S+'" value="'+op_id_S+'" > '+'</li>';	
	...
	if (localStorage.op_T.length != '') {
			...
			for (j=0; j < opLength_T; j++){
				...
				op_tbl_T=op_tbl_T+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_T+'\')"  class="name"><font id="opName'+ op_id_T +'" class="name" >'+ op_name_T+'</font><input type="hidden" id="doc_op_id'+op_id_T+'" value="'+op_id_T+'" > '+'</li>';		
	...
	if (localStorage.op_U.length != '') {
			...
			for (j=0; j < opLength_U; j++){
				...
				op_tbl_U=op_tbl_U+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_U+'\')"  class="name"><font id="opName'+ op_id_U +'" class="name" >'+ op_name_U+'</font><input type="hidden" id="doc_op_id'+op_id_U+'" value="'+op_id_U+'" > '+'</li>';	
	...
	if (localStorage.op_V.length != '') {
			...
			for (j=0; j < opLength_V; j++){
				...
				op_tbl_V=op_tbl_V+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_V+'\')"  class="name"><font id="opName'+ op_id_V +'" class="name" >'+ op_name_V+'</font><input type="hidden" id="doc_op_id'+op_id_V+'" value="'+op_id_V+'" > '+'</li>';	
	...
	if (localStorage.op_W.length != '') {
			...
			for (j=0; j < opLength_W; j++){
				...
				op_tbl_W=op_tbl_W+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_W+'\')"  class="name"><font id="opName'+ op_id_W +'" class="name" >'+ op_name_W+'</font><input type="hidden" id="doc_op_id'+op_id_W+'" value="'+op_id_W+'" > '+'</li>';	
	...
	if (localStorage.op_X.length != '') {
			...
			for (j=0; j < opLength_X; j++){
				...
				op_tbl_X=op_tbl_X+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_X+'\')"  class="name"><font id="opName'+ op_id_X +'" class="name" >'+ op_name_X+'</font><input type="hidden" id="doc_op_id'+op_id_X+'" value="'+op_id_X+'" > '+'</li>';	
	...
	if (localStorage.op_Y.length != '') {
			...
			for (j=0; j < opLength_Y; j++){
				...
				op_tbl_Y=op_tbl_Y+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_Y+'\')"  class="name"><font id="opName'+ op_id_Y +'" class="name" >'+ op_name_Y+'</font><input type="hidden" id="doc_op_id'+op_id_Y+'" value="'+op_id_Y+'" > '+'</li>';	
	...
	if (localStorage.op_Z.length != '') {
			...
			for (j=0; j < opLength_Z; j++){
				...
				op_tbl_Z=op_tbl_Z+'<li  style=" border:1px solid #CBE4E4;color:#fff "  onClick="check_boxTrue_op(\''+op_id_Z+'\')"  class="name"><font id="opName'+ op_id_Z +'" class="name" >'+ op_name_Z+'</font><input type="hidden" id="doc_op_id'+op_id_Z+'" value="'+op_id_Z+'" > '+'</li>';	
...
function uploadPhoto_docVisit(imageURI, imageName) {
   ...
   // alert (localStorage.photo_submit_url)
	...
	var apipath_image=localStorage.apipath_image+"fileUploader_docVisit/"
...
function farmImage() {
	//navigator.camera.getPicture(onSuccessProfile, onFailProfile, { quality: 10,
		//destinationType: Camera.DestinationType.FILE_URI });
   navigator.camera.getPicture(onSuccess_farmImage, onFail_farmImage, { quality: 90,
		targetWidth: 400,
		destinationType: Camera.DestinationType.FILE_URI,correctOrientation: true });
		
}
function onSuccess_farmImage(imageURI) {
	//alert ('Success')
    var image = document.getElementById('myImagefarm');
    image.src = imageURI;
	imagePath = imageURI;
	$("#farmPhoto").val(imagePath);
	

		
}
function onFail_farmImage(message) {
	//alert ('Fail')
	imagePath="";
    alert('Failed because: ' + message);
}
								...
								//		========================
									...
									// 2019-02-01 end
										...
										for (var i=0; i < FarmListLength-1; i++){
												...
												farmShow_list+='<li class="ui-btn ui-shadow ui-corner-all ui-btn-icon-left ui-icon-location" style=" border-radius:0px;border-bottom:1px solid #a5a9a9;color:#fff"><table><tr><td><img onClick="page_farm_profile(\''+farm_name+'|'+farmID+'\')" style="height:20px; width:20px" src="editProfile.png">    </td><td><a  onClick="farmVist(\''+farm_name+'|'+farmID+'\')"><font class="name" style="font-size:18; font-weight:600; color:#fff">'+farm_name+'| </font>'+farmID+'</font></a></td></tr></table></li>'	
		...
		$.ajax(localStorage.report_url+'farm_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&farm_Id='+farm_Id+'&farm_combo_val_tr='+localStorage.farm_combo_val_tr,{
			  ...
			  // alert(poandsSize)
			   if(farm_type=='POULTRY'){
					...
					$("#rearingP").val(rearing_housing)
					$("#feedingP").val(feeding)
					$("#wateringP").val(watering)
					$("#broodingP").val(brooding)
			   ...
			   if(farm_type=='CATTLE'){
					...
					$("#cat_rearing").val(rearing_housing)
			   ...
			   if(farm_type=='AQUA'){
					...
					// $("#rearingA").val(rearing_housing)
					$("#feedingA").val(feeding)
					// $("#broodingA").val(brooding)									
...
function farmVist(getData) {
	...
	$.ajax(localStorage.report_url+'farm_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&farm_Id='+farm_Id+'&farm_combo_val_tr='+localStorage.farm_combo_val_tr,{
			  ...
			  // alert(farm_type)
					$("#fv_name").html(farm_name+'|'+ farm_id)
					$.afui.loadContent("#farm_visit",true,true,'right'); 
...
function fv_submit() {
	...
	$("#error_FV_page").html('')
	$("#wait_image_farmV").show();	
	$("#farm_v_SButton").hide();	
	
	var farm_Id=localStorage.visit_farm_route.split('|')[1];
	var farm_name=localStorage.visit_farm_route.split('|')[0]
	
	var tr_owner=$("#tr_owner").val();
	var tr_visitType=$("#tr_visitType").val();
	var tr_doctor=$("#tr_doctor").val();
	var tr_rx=$("#tr_rx").val();
	var tr_note=$("#tr_note").val();
	

	//alert (localStorage.report_url+'fv_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&farm_Id='+farm_Id+'&tr_owner='+tr_owner+'&tr_visitType='+tr_visitType+'&tr_doctor='+tr_doctor+'&tr_rx='+tr_rx+'&tr_note='+tr_note)
	...
	$.ajax(localStorage.report_url+'fv_submit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&farm_Id='+farm_Id+'&tr_owner='+tr_owner+'&tr_visitType='+tr_visitType+'&tr_doctor='+tr_doctor+'&tr_rx='+tr_rx+'&tr_note='+tr_note,{
				type: 'POST',
				timeout: 30000,
				error: function(xhr) {
				 $("#wait_image_docProf").hide();
				...
				success:function(data, status,xhr){	
					// $("#wait_image_docProf").hide();
					 if (status!='success'){
						$("#wait_image_farmV").hide();	
						$("#error_FV_page").html('Network Timeout. Please check your Internet connection...');
					 ...
					 $("#farm_v_SButton").show();
						var resultArray = data.replace('</START>','').replace('</END>','').split('<SYNCDATA>');	
							...
							if (resultArray[0]=='FAILED'){
										$("#error_FV_page").text(resultArray[1]);	
...
function changePw_main(){
	
						$("#load_image").hide();
	$("#user_idp").val(localStorage.user_id);
	$("#msg_pw").text('');
	$("#msg_pw").hide();
	
    $("#load_image").hide();		
	$("#error_pass_chng").hide();	
	
	$("#old_password").val('');
	$("#new_password").val('');
	$("#confirm_password").val('');
	$("#change_pw_backhome").show()
	$.afui.loadContent("#change_pwView",true,true,'right');
}

function change_pw(){

	var user_id=$("#user_idp").val(localStorage.user_id);
	...
	if(errorFlag==1){
		$("#error_pass_chng").text("Please Complete Required Fields ");
		$("#error_pass_chng").show();
			
	}else{
	
	// $("#error_check").val(localStorage.base_url+'change_password?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&old_password='+old_password+'&new_password='+new_password+'&confirm_password='+confirm_password)		
	// alert(localStorage.base_url+'change_password?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&old_password='+old_password+'&new_password='+new_password+'&confirm_password='+confirm_password)	
	$.ajax({
		
			type:'POST',
			timeout: 30000,
			url:localStorage.base_url+'change_password?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&old_password='+old_password+'&new_password='+new_password+'&confirm_password='+confirm_password,
			success: function(data,status,xhr) {
						var resultArray = data.replace('</START>','').replace('</END>','').split('<chngPass>');	
						if (resultArray[0]=='FAILED'){

							$("#change_pw_backhome").show()
							$("#msg_pw").html(resultArray[1]);
					...
					error: function(){
						$("#load_image").hide();
						$("#btn_id").show();
						$("#error_msgp").text('Network Timeout. Please check your Internet connection.');
					}
		 });
